...
- [ ] 1.1 In `autocoder/src/lanes/walker.rs::fail_and_maybe_park`, replace the
  pair
  ```rust
...
- [ ] 1.2 Keep the existing threshold check (`if count >= perma_stuck_threshold { park_issue(...).await; }`)
  driven by the bound `count`, and keep the final `IssueStep::Failed { reason }`
  return on the success path.
- [ ] 1.3 Do not remove `state::failure_count` (it remains the lane's read
...
- [ ] 2.1 In the `autocoder/src/lanes/walker.rs` test module, add a test that
...
## 3. Verify
- [ ] 3.1 Run `cargo test -p autocoder lanes::walker` (or the crate's lane tests)
