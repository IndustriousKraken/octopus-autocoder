...
The wizard also asks whether to keep the **issues lane** on — a single `[Y/n]` gate, default yes, shown after the periodic-audits prompts. The issues lane is one of the two fundamental work paths, so it is on by default: enabling it sets per-iteration precedence `issues > changes > audits`. This does NOT turn on autonomous triage of open GitHub issues — that read-only public-issue ingestion stays separately gated by `features.scout.include_issues`. Opt out if you track corrections in an external tracker (Jira, Linear, and similar). For non-interactive installs, pass `--issues-lane <enabled|disabled>` (defaults to `enabled`); omitting it keeps the lane on, and an existing install whose config omits `features.issues` gains the active lane on upgrade. `features.*` is not part of the `autocoder reload` hot-reload set, so changing the lane after first install means editing `config.yaml` and restarting the daemon. See [docs/CONFIG.md#featuresissues](docs/CONFIG.md#featuresissues).
...
**Fix corrections via the issues lane** *(on by default; `features.issues.enabled`)*. A second work lane for *corrections* — small fixes that don't warrant a spec change — running alongside the changes queue. Two entry points: a maintainer commits `issues/<slug>.md` (or the directory form `issues/<slug>/`) at the repo root directly (curated), or the bot triages the repo's open GitHub issues read-only and posts each as a chatops *candidate* a maintainer promotes with `send it` (public ingestion, separately gated by `features.scout.include_issues`). Promotion is the authorization gate — the public can report but never trigger code work, and issue bodies are carried as untrusted data, never instructions. Promoted issues are worked into a `fix:` PR. On by default; the lane sets per-iteration precedence `issues > changes > audits`. Opt out with `features.issues.enabled: false`. See [docs/OPERATIONS.md → Issues lane](docs/OPERATIONS.md#issues-lane).
