...
- [x] Factor the shared enqueue body out of the `handle_queue_*`
...
- [x] Move the accreted handler bodies behind that dispatch table into a
...
- [x] Extract the `send it` cascade and its audit/survey state-machine
...
- [x] Preserve the canon-mandated context resolution order exactly:
...
- [x] Move the self-contained "Agentic reviewer transport (a58)" block
...
- [x] Re-export the items that callers outside the reviewer reference so
      existing paths keep compiling.

## 4. revisions.rs — collapse the duplicated outcome arms

- [x] In `process_one_pr` (`autocoder/src/revisions.rs:624-1539`), factor
...
- [x] Split the remaining body along its internal phases into smaller
...
- [x] Move the `--reconfigure` subsystem
...
- [x] Update the `execute_inner` call site in `install.rs` to call into
...
- [x] `cargo build` succeeds and the existing test suite passes (the two
