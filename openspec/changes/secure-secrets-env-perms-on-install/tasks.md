...
- [ ] 1.1 In `autocoder/src/cli/install.rs::execute_inner`, replace
  ...
  not world-readable.
- [ ] 1.2 In the same function, do the same for `config_path`:
...
- [ ] 1.3 After both files are written, the existing `chown` calls
...
- [ ] 2.2 Add a regression test asserting that — when the install
...
- [ ] 3.1 If `docs/SECURITY.md` mentions credential handling, add a
