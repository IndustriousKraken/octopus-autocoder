...
- Affected code: `lanes/issues.rs` (loader, `list_ready`, `lock`/`unlock`,
  `archive`, `issue_dir` resolution accept a file OR a directory; markers as
  siblings for the file form), `lanes/ingestion.rs` (curated candidate may draft
  a single file; public-origin candidate keeps the directory form),
  `polling_loop/review_context.rs` (the issue brief reads the unit's body),
  and the issue-flavored implementer prompt (reads the unit's body).
