...
- [ ] 1.1 Replace `SlackConfig` in `src/config.rs` with a new `ChatOpsConfig`
...
- [ ] 1.2 Define each sub-config struct per `design.md`'s schema:
...
- [ ] 1.3 Rename `Config.slack: Option<SlackConfig>` to
    `chatops: Option<ChatOpsConfig>`. Update every callsite.
- [ ] 1.4 Rename `RepositoryConfig.slack_channel_id` to
    `chatops_channel_id`. Update the `slack_channel(...)` helper to
    `chatops_channel(...)`. Update every callsite.
- [ ] 1.5 Update `config.example.yaml`: replace the commented-out `slack:`
    ...
    as EXPERIMENTAL.
- [ ] 1.6 **Verify:** `cargo test config::tests::loads_with_chatops_slack`
...
- [ ] 2.1 Convert `src/chatops.rs` into `src/chatops/mod.rs`. Move the
...
- [ ] 2.2 Define the `ChatOpsBackend` trait in `mod.rs` per design.md:
...
- [ ] 2.3 Add a startup factory `pub async fn from_config(cfg: &ChatOpsConfig)
...
- [ ] 2.4 Update `ChatOpsContext` in `polling_loop.rs` from
...
- [ ] 2.5 Update `cli/run.rs` to call `chatops::from_config(...)` instead
    of `ChatOps::new(...)`. Construction now returns
    `Arc<dyn ChatOpsBackend>` directly.
- [ ] 2.6 **Verify:** existing polling-loop tests
...
- [ ] 3.1 Move the existing Slack `ChatOps` impl into a new
...
- [ ] 3.2 Implement `ChatOpsBackend for SlackBackend`:
...
- [ ] 3.3 **Verify:** the two existing mockito tests
...
- [ ] 4.1 Create `src/chatops/discord.rs` with
...
- [ ] 4.2 Implement `post_question`: `POST /channels/{c}/messages` with
    JSON `{"content": "❓ `<change>`: <question>"}`. Return the response's
    `id` field (a snowflake string) as the handle.
- [ ] 4.3 Implement `poll_thread_for_human_reply`:
...
- [ ] 4.4 Implement `ChatOpsBackend for DiscordBackend`:
    `provider_name() -> "discord"`, `is_experimental() -> true`, plus the
    two methods above.
- [ ] 4.5 **Verify:** two mockito tests in `discord.rs`:
...
- [ ] 5.1 Create `src/chatops/teams.rs` with
...
- [ ] 5.2 Implement private `acquire_token()`:
...
- [ ] 5.3 Constructor calls `acquire_token()` to validate credentials at
    startup. Caches `bot_identity = client_id` (Teams app identity).
- [ ] 5.4 Implement `post_question`:
...
- [ ] 5.5 Implement `poll_thread_for_human_reply`:
...
- [ ] 6.1 Create `src/chatops/mattermost.rs` with
...
- [ ] 7.3 Implement `poll_thread_for_human_reply`:
...
- [ ] 7.4 Implement `ChatOpsBackend for MatrixBackend`:
    `provider_name() -> "matrix"`, `is_experimental() -> true`.
- [ ] 7.5 **Verify:** two mockito tests in `matrix.rs`:
...
- [ ] 8.1 In `cli/run.rs`, after `validate_github_token_routes`, replace
...
- [ ] 8.2 Emit the startup log line per requirement: if backend is
...
- [ ] 8.3 Update `ChatOpsContext` construction in `cli/run.rs` to use
    `repo.chatops_channel(&chatops_cfg.default_channel_id)` for the
    per-repo channel resolution.
- [ ] 8.4 **Verify:** add `cli::run::tests::startup_logs_experimental_warning_for_discord`
...
- [ ] 9.1 README: rename the "ChatOps Escalation" section's anchor and
    contents to describe the `chatops:` block with `provider: slack` as
    the officially-supported path. Update all internal links.
- [ ] 9.2 Add a new "Experimental ChatOps Backends" section immediately
    ...
    pointer for each.
- [ ] 9.3 Configuration Reference: replace the `slack:` row in the table
    with a `chatops:` row. Add a `chatops.provider` row listing valid
    values and which are experimental.
- [ ] 9.4 Quick Start prerequisites mention only Slack (the official
    path); the experimental section is for operators who deliberately
    seek it out.
- [ ] 9.5 Deployment section's `EnvironmentFile=/etc/autocoder.env`
    example: add commented-in alternatives for each experimental
    provider's required env vars.

## 10. Verification

- [ ] 10.1 `cargo test` passes with no regressions. Test count grows by
...
- [ ] 10.2 `cargo build --release` produces a binary that, given a
...
- [ ] 10.3 `openspec validate experimental-chatops-providers --strict`
    passes.
