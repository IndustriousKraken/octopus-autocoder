...
The companion `autocoder-uses-openspec-archive` change switches the per-iteration archive step to invoke `openspec archive` (so canonical specs stay in sync going forward, assuming the host has openspec sync configured). It does NOT address pre-existing drift in repos that have been autocoder-managed since before that fix landed. Any repo that accumulated archives under the prior Rust-rename code path carries unsynced requirements; any repo onboarded from a host without the openspec sync workflow configured at the time of past archives arrives with drift.

Incremental backfill (re-running `openspec archive` only on changes whose deltas aren't yet in canonical) is **fundamentally unreliable when drift is mid-history** rather than end-of-history. If change B never synced but changes C and D did, the canonical state was built from A + C + D in that order. Re-archiving B applies B's deltas on top of canonical-with-C-and-D-already-merged — but the chronologically-correct merge would have B between A and C. Subsequent MODIFIED requirements in C may have been built assuming B's contribution existed. The incremental result is silently incorrect.
...
- Breaking: no.
- Acceptance: `cargo test` passes (new tests). `openspec validate rebuild-canonical-specs-from-archive --strict` passes. Integration test: against a fixture repo with unsynced requirements, the rebuild regenerates all canonical spec files, leaves the archive directory unchanged (date prefixes preserved via in-place rename), produces a `git diff openspec/specs/` containing the merged requirements, and lists every archived change in chronological order in canonical history.
