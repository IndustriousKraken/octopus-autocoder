...
The companion `autocoder-uses-openspec-archive` change switches the per-iteration archive step to invoke `openspec archive` (so canonical specs stay in sync going forward, assuming the host has openspec sync configured). It does NOT address pre-existing drift in repos that have been autocoder-managed since before that fix landed. This autocoder repo has ~30 unsynced requirements accumulated over the past few weeks; coterie likely has a similar gap; Jeremy's repos (whenever he onboarded) too; any future repo onboarded from a host where openspec sync wasn't enabled at the time of past archives will arrive with drift.
