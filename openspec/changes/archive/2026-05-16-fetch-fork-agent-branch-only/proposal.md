...
The implementation overshot. `git fetch fork` (no refspec) fetches every branch on the fork — including branches with names that already exist on `origin`. Production failure observed on `coterie` (2026-05-16):
