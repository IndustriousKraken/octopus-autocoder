...
happens the session ends without a valid `submit_review` submission: the review
fails closed (correctly — no silent `Approve`), but the PR receives no verdict
and the operator must keep retrying or merge unreviewed. The failure is
reproducible on large PRs while smaller PRs review cleanly, which points at the
one unbounded input the agentic path still forces into every prompt: the diff.
