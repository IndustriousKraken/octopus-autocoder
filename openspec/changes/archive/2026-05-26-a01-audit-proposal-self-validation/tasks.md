...
- [x] 1.1 In `autocoder/src/config.rs`, extend `AuditsConfig` with `pub max_validation_retries: u32` (defaults to `1` via `#[serde(default = "default_max_validation_retries")]`). Add the default function returning `1`.
- [x] 1.2 Validation at config-load: clamp values above `5` to `5` with a WARN log at startup (runaway retries from operator misconfiguration is more harmful than a tight cap; the `5` ceiling is arbitrary but reasonable — operators who think they need 6+ retries probably have a different problem).
- [x] 1.3 Tests:
...
- [x] 2.1 In `autocoder/src/audits/mod.rs`, add `pub fn validate_proposal(workspace: &Path, slug: &str) -> Result<(), String>` that shells out to `openspec validate <slug> --strict` in the workspace, captures stderr, and returns:
  - Exit 0 → `Ok(())`
  - Exit non-zero → `Err(stderr.trim().to_string())`
  - Spawn failure → `Err(format!("openspec validate spawn failed: {e}"))`
- [x] 2.2 Tests using mockable subprocess invocation (or against real fixture workspaces with openspec installed; tests can skip when openspec is missing, the same pattern as `cli::sync_specs::tests::rebuild_canonical_e2e_via_openspec`):
...
- [x] 3.1 Add `pub async fn validate_with_retry<F, Fut>(workspace: &Path, slug: &str, max_retries: u32, mut llm_call: F) -> Result<RetryOutcome, ValidationExhausted>` where:
...
- [x] 4.1 Add `pub async fn discard_proposal_and_notify(workspace: &Path, slug: &str, audit_type: &str, retries_attempted: u32, final_error: &str, chatops_ctx: Option<&ChatOpsContext>) -> Result<()>`. Actions in order:
...
- [x] 4.2 Notification text format:
  ```
...
- [x] 5.1 `autocoder/src/audits/architecture_consultative.rs`: architecture_consultative produces `Reported(findings)` only; it does NOT generate proposals. Documented with a code comment; carries `retries_used: 0`.
- [x] 5.2 Same shape for `autocoder/src/audits/drift.rs` — produces `Reported(findings)` only; documented; carries `retries_used: 0`.
- [x] 5.3 `autocoder/src/audits/specs_writing.rs` wraps each LLM run in a per-attempt retry loop. Mixed-validity outputs preserve the existing per-change drop behavior (no retry); all-invalid runs trigger retries (with the LLM addendum) and ultimately `ValidationExhausted` if the budget is exhausted.
- [x] 5.4 The `architecture_brightline` audit does NOT generate proposals via LLM (pure-data file-line-counting). Documented in a code comment.
- [x] 5.5 Tests per audit type:
...
- [x] 6.1 Extend `pub enum AuditOutcome` in `autocoder/src/audits/mod.rs` with:
  ```rust
  ValidationExhausted {
      audit_type: String,
  ...
  ```
- [x] 6.2 Update the existing `Reported` variant (or whatever the LLM-driven audits use for success) to carry `retries_used: u32` so success-with-retry can be distinguished from success-on-first-attempt in logs and the chatops notification.
- [x] 6.3 Update the scheduler's outcome handling at `autocoder/src/audits/scheduler.rs` to:
...
- [x] 7.1 Each audit type's state file (under `<workspace>/.autocoder/audit-state/<audit-type>.json` or wherever the existing state lives) gains an `attempt_history: Vec<AttemptEntry>` field where each entry records:
  ```rust
...
- [x] 8.1 Add a paragraph in `docs/CONFIG.md`'s `audits:` reference section documenting the new `max_validation_retries` field (default `1`, max `5`, semantics).
- [x] 8.2 Add a paragraph in `docs/CHATOPS.md` documenting the new `❌ <audit-type> produced an invalid proposal` notification — when it fires, what it means, what the operator should do (typically: review the audit's prompt template if the audit-type fails validation repeatedly).
- [x] 8.3 Add an entry in `docs/TROUBLESHOOTING.md` for the new failure mode: "Audit produces invalid proposal — what to do." Cross-reference the related cascade-prevention specs (`queue-archive-aborted-detection`, `pr-body-proposal-active-path-fallback`).
...
- [x] 10.1 `cargo test` passes (new + existing). 924 passed, 2 ignored, 0 failed.
- [x] 10.2 `openspec validate a01-audit-proposal-self-validation --strict` passes.
- [x] 10.3 `cargo clippy --all-targets --all-features` produces no new warnings (count is net-down vs baseline; pre-existing warnings unrelated to this change remain).
