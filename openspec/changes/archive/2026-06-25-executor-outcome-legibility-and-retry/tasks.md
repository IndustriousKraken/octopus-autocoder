...
- [x] 1.1 In `autocoder/src/executor/claude_cli.rs`, replace the failure-reason
...
- [x] 1.2 Reuse / extend `agentic_run::failure_excerpt(outcome, max)`
  ...
  it is surfaced raw.
- [x] 1.3 Confirm the timeout path still yields its existing terminal reason
  (`timed_out` → a clear "timeout" reason) and is not regressed by the assembler.

## 2. Optional, strategy-local retry hint (`CliStrategy`)

- [x] 2.1 In `autocoder/src/agentic_run.rs`, add to `trait CliStrategy`
...
- [x] 3.1 In `autocoder/src/config.rs`, add `executor.session_retries: u32`
...
- [x] 3.2 In the pass/outcome path (`autocoder/src/polling_loop/outcome.rs`), when
...
- [x] 3.3 A session that produced a committable result is NOT retried (it is a
...
- [x] 3.4 Keep the existing whole-pass scheduling intact: this is an in-pass
  bounded retry, not a change to the daemon's next-pass re-pickup behavior.

## 4. Notification carries the assembled reason

- [x] 4.1 Ensure the operator-facing failure notifications render the assembled
...
- [x] 5.1 Reason assembly (`agentic_run` / `claude_cli` tests): given an outcome
  ...
  exact wording.
- [x] 5.2 Bounded retry (`polling_loop` tests): a session that fails with no
...
- [x] 5.3 `executor.session_retries` config: default applied when absent; explicit
...
- [x] 6.2 `openspec validate executor-outcome-legibility-and-retry --strict` from
  the repo root.

## 7. Reviewer no-submission discard surfaces + persists its output

- [x] 7.1 In `autocoder/src/code_reviewer.rs`, on the no-submission path where the
...
- [x] 7.2 Persist the reviewer session's captured output to a discoverable
...
- [x] 7.3 Test: a reviewer session that records no submission with non-empty
