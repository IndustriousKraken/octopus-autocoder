...
Observed twice in production (coterie + openspec-autocoder, 2026-05-16 00:53 / 00:56) after `/tmp` directory cleanup. The fix is to fetch the fork remote at clone time so the local tracking ref reflects reality immediately, and `--force-with-lease`'s safety check operates on accurate data.
