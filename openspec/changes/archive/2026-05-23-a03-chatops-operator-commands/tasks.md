...
- [x] 2.2 Case-insensitive substring against `repository.url`. The match is liberal: `coterie` matches `git@github.com:IndustriousKraken/coterie.git`. If two repos with the same name exist under different owners, the operator must type more characters to disambiguate.
