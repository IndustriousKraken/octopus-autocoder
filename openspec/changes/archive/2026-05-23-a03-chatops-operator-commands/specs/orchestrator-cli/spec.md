...
- **WHEN** an operator posts `@<bot> status coterie` in a
  channel where the chatops listener is active AND `coterie`
...
- **AND** if `coterie` matches multiple configured repos, the
...
- **WHEN** an operator posts
  `@<bot> clear-perma-stuck coterie a06-foo`
...
- **AND** on marker-not-found: the bot posts
  `✗ no perma-stuck marker for change a06-foo on coterie`
  (informational; not retried)

#### Scenario: clear-revision removes the spec-revision marker
- **WHEN** an operator posts
  `@<bot> clear-revision coterie a07-bar`
- **THEN** the bot resolves the repo, submits a
...
- **WHEN** an operator posts `@<bot> wipe-workspace coterie`
  in channel `C` AND `coterie` resolves to a unique repo
...
- **WHEN** an operator posts `@<bot> status COTERIE`,
  `@<bot> status Coterie`, or `@<bot> status coterie`
...
- **WHEN** an operator posts `@<bot> pause coterie` (or
