...
- [x] 1.1 Create `autocoder/src/paths.rs`. Public surface:
  ```rust
  ...
  ```
- [x] 1.2 Resolution priority per field:
...
- [x] 1.3 Validation at resolution time:
...
- [x] 2.1 In `autocoder/src/config.rs`, add a top-level optional `paths:` block:
  ```rust
...
- [x] 3.1 In `autocoder/src/workspace.rs`, replace the hard-coded `/tmp/workspaces/` derivation with `daemon_paths.cache.join("workspaces")`. The deterministic-sanitization rule (URL → directory name) is unchanged. Callers thread `DaemonPaths` through (or grab from a process-global lazy `OnceCell` populated at startup; pick whichever fits the existing dependency injection pattern best).
- [x] 3.2 In `autocoder/src/control_socket.rs`, replace `/tmp/autocoder/control/control.sock` with `daemon_paths.runtime.join("control.sock")`. The CLI's reload-via-socket lookup uses the same resolution.
- [x] 3.3 In `autocoder/src/audits/scheduler.rs` (and any other audit modules that write state), the audit-state file path becomes `daemon_paths.state.join("audit-state").join(format!("{audit_type}.json"))`. Per-audit-type aggregated files are written by the daemon under the new layout; the per-workspace `.audit-state.json` continues to be loaded by the iteration's scheduler for backwards compatibility with existing fixtures.
- [x] 3.4 In `autocoder/src/failure_state.rs`, the failure-state file path becomes `daemon_paths.state.join("failure-state").join(repo_sanitized).join(format!("{change}.json"))`. The repo_sanitized form uses the same URL-sanitization rule as workspaces (consistency). A workspace-local fallback path is used in tests that have not installed the global `DaemonPaths`.
- [x] 3.5 In `autocoder/src/revisions.rs` (from `a01-pr-comment-revision-loop`), the per-PR state file path becomes `daemon_paths.state.join("revisions").join(repo_sanitized).join(format!("{pr_number}.json"))`. A workspace-local fallback path is used in tests that have not installed the global `DaemonPaths`.
- [x] 3.6 Per-change run logs at `daemon_paths.logs.join("runs").join(repo_sanitized).join(format!("{change}.log"))`. Update the executor's log-writer construction.
- [x] 3.7 Any in-progress lock files / per-process pid files move to `daemon_paths.runtime`. These are ephemeral by design; living in /run (or `$XDG_RUNTIME_DIR`) means reboot cleans them up automatically. (`busy_marker` and subprocess sidecar PIDs now resolve under `<runtime_dir>/busy/`.)
- [x] 3.8 Tests for each callsite: with a fixture `DaemonPaths` pointing at a tempdir, the write produces a file at the expected sub-path under that tempdir.

## 4. Audit-state reload on startup

- [x] 4.1 In `autocoder/src/audits/scheduler.rs` (or wherever the AuditState in-memory map is owned), add `pub fn reload_from_disk(state_dir: &Path) -> Result<HashMap<String, AuditState>>` that:
...
- [x] 4.2 Call `reload_from_disk` at daemon start, BEFORE any audit cadence check fires. The result populates the in-memory map; subsequent cadence checks read from that map.
- [x] 4.3 Tests:
...
- [x] 5.1 Create `autocoder/src/migration.rs`. Public surface:
  ```rust
  ...
  ```
- [x] 5.2 Migration flow on each call:
  ...
  5. Return the report.
- [x] 5.3 Logging: per source-entry, log at INFO the source + target paths. At end, log a summary line with the report's counts and any error count. ERRORs log per-entry at ERROR with the source path and the OS error.
- [x] 5.4 Tests:
...
- [x] 6.1 In `autocoder/src/cli/run.rs`, at daemon start:
...
- [x] 6.2 Migration failures (the report containing errors) are LOGGED but do not abort startup. The daemon proceeds with whatever state is in place. Operators see the ERROR lines in journalctl and can investigate / clean up orphan /tmp entries manually.
- [x] 6.3 Tests:
  - End-to-end fixture: tempdir as daemon-paths root, populate /tmp with legacy fixture data (or mock the legacy-path constants), run daemon-start path, assert the migration ran, the marker was written, the daemon continues to its normal init. (Covered by `migration::tests` which drives the same `migrate_at` entry point that `cli::run::execute` invokes; the daemon startup wrapper just logs the report.)

## 7. systemd unit template updates

- [x] 7.1 In `autocoder/src/cli/install_systemd.service`, add under `[Service]`:
  ```
  ...
  ```
- [x] 7.2 Adjust the existing `ReadWritePaths=` directive to reflect the new directories (the directives systemd auto-creates are already RW for the service user; ReadWritePaths can be tightened or removed accordingly). Removed `ReadWritePaths=/var/lib/autocoder /tmp`; the four `*Directory=autocoder` directives auto-create RW paths under the service user.
- [x] 7.3 The install wizard re-renders the unit when an upgrade install is run; operators on the OLD unit can `autocoder install --upgrade` to refresh, OR manually edit their unit to add the four directives.
- [x] 7.4 Tests:
  - Render the unit template, assert it contains the four `*Directory=autocoder` lines.

## 8. Install wizard updates

- [x] 8.1 Server mode: rendered unit gains the four directives (covered by task 7).
- [x] 8.2 Dev mode: the install wizard computes XDG paths and writes them into the generated `config.yaml`'s `paths:` block so operators see the resolved values explicitly. Helps debuggability ("where is my state actually written?").
- [x] 8.3 Tests:
...
- [x] 9.1 Add `docs/STATE-LAYOUT.md` (or section in `docs/OPERATIONS.md`) describing:
...
- [x] 9.2 Update `docs/CONFIG.md` with the new `paths:` block reference.
- [x] 9.3 Update `docs/DEPLOYMENT.md` if it currently mentions `/tmp/workspaces/...` paths anywhere (it almost certainly does).
- [x] 9.4 Update `docs/TROUBLESHOOTING.md` with an entry for "audit storm after reboot" pointing operators at the audit-state reload mechanism — if a storm still happens after this change ships, the operator should check that their daemon actually picked up the new paths (look for the migration log line at startup; if absent, the daemon didn't migrate, check `paths:` config or env vars).
...
- [x] 11.1 `cargo test` passes (new + existing). 1104 tests pass, 0 fail.
- [x] 11.2 `openspec validate state-paths-out-of-tmp --strict` passes.
- [x] 11.3 `cargo clippy --all-targets --all-features -- -D warnings` produces no new warnings. (Pre-existing warnings on the codebase are unchanged; the lints my new code introduced have been fixed.)
