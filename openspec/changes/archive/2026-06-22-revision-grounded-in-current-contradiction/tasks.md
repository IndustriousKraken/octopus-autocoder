...
- [x] 1.1 In `autocoder/src/spec_revision.rs`, extend the marker so it can carry
...
- [x] 2.1 In `autocoder/src/polling/revision_session.rs`, change `ReGateOutcome`
...
- [x] 3.1 In `run_revision_execute`, on `ReGateOutcome::Contradiction(...)`,
...
- [x] 3.2 The refresh updates findings only — it does NOT clear the marker and is
...
- [x] 4.1 In `build_executor_prompt`, read the marker's structured findings and
...
- [x] 5.1 In `process_pending_revision_execute`, wrap `fetch_thread_transcript`
...
- [x] 5.2 On persistent failure, do NOT degrade to an empty transcript and run the
...
- [x] 5.3 The read-only advisor (`process_pending_revision_advise`) keeps
...
- [x] 6.1 Add config `executor.revision_converge_attempts` (additional in-`send
  ...
  config fields.
- [x] 6.2 In `run_revision_execute`, wrap the edit → scope-check → re-gate in a
...
- [x] 6.3 Escalation: track the re-gate finding identity across iterations (and,
...
- [x] 7.1 Marker refresh (task 3): an injected re-gate runner returning a
...
- [x] 7.2 Fail-closed transcript (task 5): a transcript runner that fails every
  ...
  wording.
- [x] 7.3 Resolve-all grounding (task 4): given a marker with two structured
...
- [x] 7.4 Converge loop (task 6): an injected re-gate runner returning
...
- [x] 7.5 Escalation (task 6.3): a re-gate runner returning the SAME finding
...
- [x] 7.6 Config: `revision_converge_attempts` and
...
- [x] 8.1 `cd autocoder && cargo test --bin autocoder` (the suite is known-flaky
  under parallel load — re-run / isolate any failure before treating it as real).
- [x] 8.2 `openspec validate revision-grounded-in-current-contradiction --strict`
  from the repo root.
