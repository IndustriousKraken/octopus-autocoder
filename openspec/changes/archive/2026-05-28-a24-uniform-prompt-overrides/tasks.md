...
- [x] 1.1 New module `autocoder/src/prompts/loader.rs` with:
...
- [x] 1.2 Each `PromptId` variant maps via an internal table to:
...
- [x] 1.3 Loader precedence per call:
...
- [x] 1.4 One-shot WARN tracking: an internal `HashSet<(PromptId, PathBuf)>` records WARNs already emitted so missing paths don't spam logs across reloads.
- [x] 1.5 Tests:
...
- [x] 2.1 In `autocoder/src/config.rs`, extend `PerRepoConfig.executor` with:
...
- [x] 3.1 Audit consumers: replace direct `include_str!` calls in each audit's `run()` with `PromptLoader::load(PromptId::Audit<X>, &workspace_config)`.
- [x] 3.2 Executor consumers:
...
- [x] 3.3 Code reviewer: `prompts/code-review-default.md` resolution → `PromptLoader::load(PromptId::CodeReview, ...)`.
- [x] 3.4 Brownfield handler (added by `a23`): `PromptLoader::load(PromptId::BrownfieldDraft, ...)`.
- [x] 3.5 Tests: each refactored consumer still passes its existing tests; loader integration is verified via at least one end-to-end test per consumer.

## 4. Docs

- [x] 4.1 In `docs/CONFIG.md`, add a `## Prompt overrides` section near the existing `audits.settings.<slug>.prompt_path` discussion. Contents:
...
- [x] 4.2 In `README.md`, add a single sentence in the "Configuration" section pointing operators at the `docs/CONFIG.md` prompt overrides table.
- [x] 4.3 In `config.example.yaml`, include the three new override blocks commented out, with comments showing the per-workspace path semantics.

## 5. Spec deltas

- [x] 5.1 `openspec/changes/a24-uniform-prompt-overrides/specs/executor/spec.md` ADDs the uniform-load-semantics requirement AND the three new override-field requirements.
- [x] 5.2 `openspec/changes/a24-uniform-prompt-overrides/specs/orchestrator-cli/spec.md` ADDs the triage-prompts-honor-overrides requirement.
- [x] 5.3 `openspec/changes/a24-uniform-prompt-overrides/specs/project-documentation/spec.md` ADDs the CONFIG.md prompt-overrides-table requirement.

## 6. Verification

- [x] 6.1 `cargo test` passes (new + existing).
- [x] 6.2 `openspec validate a24-uniform-prompt-overrides --strict` passes.
