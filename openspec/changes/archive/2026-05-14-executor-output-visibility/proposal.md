...
Production symptom (2026-05-14, gilgamesh24): two consecutive Failed-no-diff outcomes in the same pass for unrelated changes, indicating a systemic problem (likely sandbox config, missing openspec install, or environment-related). The operator cannot diagnose without re-running by hand or attaching a tracer.
