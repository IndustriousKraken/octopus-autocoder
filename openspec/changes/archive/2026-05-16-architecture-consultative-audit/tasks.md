...
- [x] 1.1 New file `prompts/architecture-consultative.md`. Contents:
...
- [x] 2.1 New module `autocoder/src/audits/architecture_consultative.rs`. Define `pub struct ArchitectureConsultativeAudit { settings: AuditSettings, executor_command: String, executor_timeout_secs: u64 }`.
- [x] 2.2 `impl Audit` with `audit_type() = "architecture_consultative"`, `requires_head_change() = true`, `write_policy() = WritePolicy::None`.
- [x] 2.3 `run(&self, ctx)` follows the drift-audit shape:
...
- [x] 2.4 If the drift-audit landed first with a shared LLM-audit helper, reuse it here. Otherwise consider extracting after both land. (Drift audit not landed; left for follow-up extraction once both LLM-audit shapes coexist.)
- [x] 2.5 Tests `audits::architecture_consultative::tests`:
...
- [x] 3.1 In `cli/run.rs::build_audit_registry`, append `Arc::new(ArchitectureConsultativeAudit::new(&audit_settings, &cfg.executor))`. (Deferred: `build_audit_registry` is introduced by `periodic-audits-foundation`, which has not landed yet. The audit type and constructor are in place; foundation will append its `Arc::new(...)` call when wiring the registry.)

## 4. Documentation

- [x] 4.1 README "Periodic audits" — add `architecture_consultative` to the registered-audits list. Document the consultative nature, the recommended `monthly`/`quarterly` cadence, the prompt's anti-pattern guardrails.
- [x] 4.2 README operator-guidance: "If the consultative audit produces noisy output, tighten the prompt (operator override at `audits.architecture_consultative.prompt_path`) before reaching for `disabled`. The prompt's anti-pattern list exists specifically to mitigate common LLM failure modes; if the output still misfires, the prompt is where to fix it."

## 5. Verification

- [x] 5.1 `cargo test` passes.
- [x] 5.2 `openspec validate architecture-consultative-audit --strict` passes.
