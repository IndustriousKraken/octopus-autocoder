...
- [x] 5.2 In `autocoder/src/control_socket/actions.rs`, add `SyncUpstreamAction` variant.
- [x] 5.3 New module `autocoder/src/polling/sync_upstream.rs` exposing `handle_sync_upstream(action, repo_state, daemon_ctx) -> Result<()>`. Behavior:
