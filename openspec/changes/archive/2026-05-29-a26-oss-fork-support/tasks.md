...
- [x] 2.3 Refactor every existing call site that constructs paths under `<workspace>/openspec/...`:
  ...
  - [x] `openspec validate` invocation paths → archive-time invocations run from `spec_root.spec_git_workspace()` for brownfield artifacts (see `polling/brownfield.rs::finalize_completed`). For normal-change self-heal probes, `openspec_validate_strict_passes` is now invoked with `spec_workspace` (resolved at the `handle_outcome` site) so the validate runs against the spec_root tree when spec_storage is configured.
  ...
  - [x] `openspec archive` invocations for spec-only flows → `queue::archive`, `queue::list_pending`, `queue::list_waiting`, `queue::find_queue_blocking_markers`, `queue::clear_stale_locks`, `queue::lock`/`unlock`, `queue::archive_collision_path`, AND `queue::would_collide_on_archive` keep their `workspace: &Path` signatures (they're pure path utilities). Production call sites in `polling_loop.rs` (run_pass_through_commits, process_waiting_changes, handle_outcome, process_audit_triages, process_chat_request_triages), `polling/brownfield.rs`, `changelog_triage.rs`, AND `control_socket.rs` (handle_status, handle_clear_perma_stuck, handle_clear_revision, handle_ignore_for_queue, handle_clear_ignore_for_queue) all resolve `spec_workspace = spec_root::spec_git_workspace(repo, workspace)` AT the call site AND pass that to queue. Same call-site pattern routes `queue::unarchive` (cli/rewind.rs), `spec_revision::write_marker` (polling_loop.rs handle_outcome + 3 preflight sites), `build_commit_subject`, `build_pr_body`, AND every marker-path display string (`.perma-stuck.json`, `.needs-spec-revision.json`, `.ignore-for-queue.json`).
  ...
  - **Sites intentionally NOT routed (operator picks the tree):** `cli/sync_specs.rs` AND `cli/changelog.rs` both take a workspace path directly from the operator's clap args — the operator chooses whether to point at the code workspace or the spec_storage tree, so no resolver is needed.
  - **Sites left for phase-2 architectural rework:** `mcp_askuser_server.rs` (the askuser marker path uses `ENV_WORKSPACE` set by `claude_cli::write_mcp_config` to the subprocess CWD; routing the marker to spec_storage requires either a new env var OR an Executor trait change). `executor/claude_cli.rs` self-marker reads in `run`/`resume`/`run_revision` (same reason). `log_retention.rs` (operates on filesystem discovery without per-repo config). `audits/specs_writing.rs` (the audit's LLM produces change dirs as a subprocess side effect in the CWD; routing those to spec_storage requires either a symlink overlay OR moving dirs post-execution — same architectural concern as the normal-change implementer commit-splitting in task 3.3). These are documented limitations for v1.
- [x] 2.4 Tests:
...
- [x] 3.3 Tests:
  ...
  - **Normal-change archive routing — call-site refactor completed (v1):** every queue/marker call in production code now resolves `spec_workspace = spec_root::spec_git_workspace(repo, workspace)` AT the call site. With `spec_storage` configured, queue enumeration, archive moves, lock/unlock, AND marker writes target the external spec_storage tree consistently. Brownfield + audit-triage spec writes commit there too. **Remaining v1 limitation:** the normal-change implementer subprocess still runs in the code workspace AND writes both code AND spec deltas to that single tree. So with `spec_storage` configured, an iteration's pending-queue walk SHALL find no normal-change work in spec_storage (the implementer hasn't been refactored to write spec deltas there), but it WILL pick up brownfield AND audit-triage runs that DO write to spec_storage. Splitting the implementer's single-tree contract is phase-2 architectural work tracked separately; the v1 sensible interpretation is: operators using `spec_storage` use brownfield (spec-only by construction) OR keep the openspec/ tree in the code workspace.
