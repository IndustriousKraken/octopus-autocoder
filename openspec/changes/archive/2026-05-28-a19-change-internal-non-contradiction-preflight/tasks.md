...
- [x] 1.1 In `autocoder/src/config.rs`, extend `ExecutorConfig`:
  ...
  ```
- [x] 1.2 Define the enum + sub-config:
  ```rust
  ...
  ```
- [x] 1.3 When the check is `Enabled` AND no `change_internal_contradiction_check_llm` is set, startup config-validation MUST fail with `executor.change_internal_contradiction_check is enabled but executor.change_internal_contradiction_check_llm is not configured`. Fail-fast at startup, not at the per-change check time.
- [x] 1.4 Update `config.example.yaml` AND the project-documentation config-coverage test list.
- [x] 1.5 Tests: default parses (disabled); enabled-without-llm-config fails startup validation with the expected message; enabled-with-llm-config passes validation.

## 2. Embedded prompt template

- [x] 2.1 Create `prompts/change-contradiction-check.md` per the proposal's prompt text.
- [x] 2.2 Embed via `include_str!("../../prompts/change-contradiction-check.md")`.
- [x] 2.3 The override config `change_internal_contradiction_check_prompt_path` resolves at use time. Empty override file → error at use, naming the path (don't feed an empty prompt to the LLM).
- [x] 2.4 Test: embedded template loads; override path replaces it; empty override file rejected.

## 3. Pre-flight check function

- [x] 3.1 New module `autocoder/src/preflight/change_contradiction.rs`. Public surface:
  ```rust
...
- [x] 4.1 In the polling loop's per-change pre-executor pipeline, AFTER `a17`'s archivability check AND BEFORE `executor.run(...)`:
...
- [x] 4.2 The marker's `unarchivable_deltas` field (from `a17`) is left EMPTY for this case — the issue isn't an unarchivable delta. The `unimplementable_tasks` field is also empty. The contradictions narrative goes entirely into `revision_suggestion`.
- [x] 4.3 Tests:
...
- [x] 5.1 When the contradiction check produces findings, the marker's `revision_suggestion` text is auto-generated:
  ```
  ...
  ```
- [x] 5.2 The narrative is informational — the contradiction check is opt-in AND new; operators should be able to disagree with the LLM AND clear the marker without editing if they assess it as a false positive.
- [x] 5.3 Test: marker file written with N findings has the expected `revision_suggestion` text.

## 6. Docs

- [x] 6.1 In `docs/CONFIG.md`'s `executor:` table, add rows for `change_internal_contradiction_check`, `change_internal_contradiction_check_prompt_path`, AND `change_internal_contradiction_check_llm`.
- [x] 6.2 In `docs/OPERATIONS.md`'s "Spec marked as needing revision" section, add a paragraph describing the contradiction-check failure mode AND the opt-in posture. Note that the check is LLM-based AND has a small per-change cost; operators trading cost for safety can enable it.
- [x] 6.3 In `docs/OPERATIONS.md`, add a "Pre-flight checks" section enumerating the layered pre-executor checks: `openspec validate --strict` (well-formedness), `a17`'s archivability check (mechanical), AND `a19`'s contradiction check (LLM, opt-in). Each check's purpose AND cost.

## 7. Spec deltas

- [x] 7.1 `openspec/changes/a19-change-internal-non-contradiction-preflight/specs/orchestrator-cli/spec.md` ADDs `Change-internal contradiction pre-flight check (opt-in)`.
- [x] 7.2 `openspec/changes/a19-change-internal-non-contradiction-preflight/specs/project-documentation/spec.md` ADDs `CONFIG.md and OPERATIONS.md document the contradiction-check fields and cost model`.

## 8. Verification

- [x] 8.1 `cargo test` passes (new + existing).
- [x] 8.2 `openspec validate a19-change-internal-non-contradiction-preflight --strict` passes.
