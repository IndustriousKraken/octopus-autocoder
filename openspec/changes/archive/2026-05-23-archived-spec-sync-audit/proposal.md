...
OpenSpec 0.18 split the spec-merge step out of `openspec archive` into a separate `/opsx:sync` skill. Per issue Fission-AI/OpenSpec#913, the archive workflow's "sync now" prompt points at a skill (`openspec-sync-specs`) that the core profile doesn't install, so in practice `openspec archive` archives the change directory cleanly but never propagates the change's `## ADDED` / `## MODIFIED` / `## REMOVED` requirements into the canonical `openspec/specs/<capability>/spec.md` files. Upstream is expected to re-bundle the sync skill by default in a future release, but autocoder shouldn't depend on that fix. Three reasons:

1. **The fix's timing is out of autocoder's control.** Meanwhile autocoder is running against multiple operator-managed repositories, every one of which has been silently accumulating drift since archive operations started.
...
**Backfill IS automatic**. The audit walks ALL archived changes, not just recent ones. On a repo with historical drift (like this repo's 30-requirement gap), the audit's first run produces a single large merge commit that closes the entire backfill. No separate `autocoder sync-specs --backfill` subcommand needed; the audit IS the backfill on first run.
...
- Across-projects impact: this fixes the drift everywhere autocoder runs (operator-managed repositories, future onboarded repos). The audit is enabled per-repo via the existing `audits.defaults` / `repositories[].audits` cadence config — no per-repo code changes needed.
- Breaking: no.
- Forward-compatibility with upstream OpenSpec fix: when OpenSpec re-bundles sync, `openspec archive` will start syncing again. This audit becomes mostly-noop in that world (it only finds drift introduced by sources OTHER than autocoder's archive operations, e.g. operator manual `openspec archive` runs without the skill installed, or pre-existing drift in newly-onboarded repos). The audit stays useful as a defensive backstop.
