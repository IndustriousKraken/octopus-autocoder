...
- [x] 1.1 In `autocoder/src/git.rs`, add a private function
  ...
  `JoinHandle`s.
- [x] 1.2 In the helper, loop on `child.try_wait()` with the existing
...
- [x] 1.3 On `Ok(Some(status))`, `join()` both reader threads to
...
- [x] 1.4 On deadline exceeded, call `child.kill()` then
...
- [x] 1.5 On `Err(e)` from `try_wait()`, return
...
- [x] 2.1 In `autocoder/src/git.rs::fetch_remote_with_timeout`, keep
...
- [x] 2.2 Map the returned `Output` exactly as today: on
...
- [x] 3.1 Add a unit test in the `#[cfg(test)]` module of
  ...
  then exits non-zero.
- [x] 3.2 Call `wait_capture_with_timeout(child, "test fetch", 30)` and
