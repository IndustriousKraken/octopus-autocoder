...
- [ ] 2.3 If `queue::archive` or the commit fails, log ERROR with the underlying error and return `Failed` — the change stays pending and the next pass retries (or human investigates). Self-heal is best-effort; an archive-step failure is not catastrophic.

## 3. PR body annotation for self-heal passes

- [ ] 3.1 The PR body for a pass that includes self-healed changes needs the leading paragraph: `_This PR archives one or more changes whose implementation was already present on the base branch. No code diff is included; only the openspec archive move._` Easiest approach: track a `bool` (`includes_self_heal`) alongside `processed: Vec<String>` through `run_pass_through_commits` → `execute_one_pass` → `build_pr_body`. When set, prepend the paragraph to the existing body.
- [ ] 3.2 If a pass mixes self-heal changes AND normally-implemented changes, the paragraph still appears once at the top — operators reading the PR see the disclaimer regardless.

## 4. Tests

- [ ] 4.1 `polling_loop::tests::tasks_md_all_complete_*` — unit tests for the helper:
    - All `[x]` → true
    - Mix of `[x]` and `[ ]` → false
    - All `[ ]` → false
    - Empty `tasks.md` (no `- [` lines) → false
    - Missing file → Err
- [ ] 4.2 `polling_loop::tests::self_heal_archives_when_preconditions_met` — fixture workspace where:
...
- [ ] 4.3 `polling_loop::tests::self_heal_falls_through_to_failed_when_tasks_incomplete` — same fixture but one task is `[ ]`. Assert: change is NOT archived, change is back in pending, outcome is Failed.
- [ ] 4.4 `polling_loop::tests::self_heal_falls_through_when_openspec_validate_fails` — same fixture but `openspec validate` will error (e.g. `tasks.md` malformed). Assert: change is NOT archived, outcome is Failed.
- [ ] 4.5 `polling_loop::tests::self_heal_paragraph_omitted_when_no_self_heals_in_pass` — fixture with a normal Completed-with-diff change AND no self-heal change. Assert: PR body does NOT include the self-heal disclaimer.

## 5. Documentation

- [ ] 5.1 README "Operating Notes": add a brief subsection "Self-heal for already-implemented changes" describing the trigger criteria, what the resulting PR looks like, and that the disclaimer paragraph identifies these passes. No kitschy framing.

## 6. Verification

- [ ] 6.1 `cargo test` passes.
- [ ] 6.2 `openspec validate self-heal-already-implemented --strict` passes.
