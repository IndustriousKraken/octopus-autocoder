...
- [ ] 2.2 Case-insensitive substring against `repository.url`. The match is liberal: `myrepo` matches `git@github.com:acme/myrepo.git`. If two repos with the same name exist under different owners, the operator must type more characters to disambiguate.
