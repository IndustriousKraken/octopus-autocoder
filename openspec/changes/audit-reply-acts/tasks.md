...
- [ ] 2.1 Create `autocoder/src/audits/threads.rs`. Public surface:
  ```rust
...
- [ ] 2.2 The findings excerpt is capped at 35,000 chars (same as the threaded-notification body cap from `chatops-audit-findings-in-threads`). Stored verbatim so the triage prompt can ship the full content the operator saw.
- [ ] 2.3 Tests:
...
- [ ] 3.1 In `autocoder/src/audits/scheduler.rs`, when an audit's findings post via the threaded path (per `chatops-audit-findings-in-threads`), capture the returned `thread_ts` from the chatops backend's threaded-post call. Today's `post_notification_with_thread` returns `Result<()>` — extend it to return `Result<Option<String>>` where `Some(thread_ts)` names the top-level message's id. For backends that don't support threading (default impl), return `Ok(None)` (no thread; nothing to track).
- [ ] 3.2 When the backend returns `Some(thread_ts)`, the scheduler writes an `AuditThreadState` with `status: Open` to the audit-threads directory.
- [ ] 3.3 Tests:
...
- [ ] 4.1 In `OperatorCommandDispatcher::handle_message`, add a match arm for `SendItOnAudit { thread_ts }`:
...
- [ ] 5.1 Create new template `prompts/audit-triage.md`. Required substitutions: `{{findings}}`, `{{audit_type}}`, `{{repo_url}}`, `{{canonical_specs_index}}` (a brief listing of which specs exist in `openspec/specs/`). Template instructions:
...
- [ ] 5.2 Add `TriageContext` to the executor's prompt-build path:
  ```rust
  ...
  ```
- [ ] 5.3 New executor method `run_triage(workspace, ctx) -> Result<ExecutorOutcome>`. Substitutes the template's variables, spawns the wrapped CLI, returns the outcome.
- [ ] 5.4 Tests:
...
- [ ] 6.1 In `autocoder/src/polling_loop.rs::run`, at the start of each iteration AFTER the revision-loop processing AND BEFORE the pending-change walk, drain the per-repo triage queue. For each pending `thread_ts`:
...
- [ ] 6.2 Diff split + PR creation. New `pub fn process_completed_triage(workspace, audit_thread_state, ctx, github_cfg) -> Result<TriageOutcome>`:
...
- [ ] 7.1 At each iteration's start (or once per day; pick whichever fits the existing periodic-housekeeping pattern), call `prune_stale_entries(state_dir, Duration::from_days(7))`. Removes audit-thread state files older than 7 days regardless of status.
- [ ] 7.2 Tests:
...
- [ ] 8.1 In `docs/CHATOPS.md`, add a section "Acting on an audit's findings: `send it`" describing the verb, the audit-thread tracking, the two-PR output shape, and the 7-day staleness rule.
- [ ] 8.2 In `docs/OPERATIONS.md`'s audits section, add a paragraph describing the audit → review → `send it` → fixes-PR + spec-PR → revise loop. Cross-reference `a01-pr-comment-revision-loop`'s revision-comment workflow for correcting either PR.
- [ ] 8.3 In `docs/TROUBLESHOOTING.md`, add entries for the polite-refusal cases (untracked thread; stale thread; already-acted thread) so operators reading the bot's `✗` replies know what each means.

## 9. Spec delta

- [ ] 9.1 The ADDED requirement in `openspec/changes/audit-reply-acts/specs/orchestrator-cli/spec.md` codifies: the `send it` verb in audit threads, the audit-thread state tracking, the triage-mode executor invocation, the diff-split + two-PR shape, the staleness rule, and the state-transition diagram.

## 10. Verification

- [ ] 10.1 `cargo test` passes (new + existing).
- [ ] 10.2 `openspec validate audit-reply-acts --strict` passes.
