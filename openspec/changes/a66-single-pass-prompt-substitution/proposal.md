...
- **Acceptance:** `cargo test` passes; `openspec validate a66-single-pass-prompt-substitution --strict` passes. Tests: a value containing another placeholder token is not re-expanded; rendered size grows linearly (not multiplicatively) with the count of placeholder literals in injected values; all placeholders still substitute once for normal inputs; a `ReviewContext` whose changed files contain literal `{{diff}}`/`{{changed_files}}` renders a prompt bounded by the section sizes.
