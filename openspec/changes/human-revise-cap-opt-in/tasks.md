...
- [ ] 1.1 In `autocoder/src/config.rs`, change `max_revise_triggers_per_pr` from
...
- [ ] 2.1 In the revise dispatcher (`autocoder/src/revisions.rs`), the cap check
...
- [ ] 2.2 Leave the auto-revision cap (`max_auto_revisions_per_pr`) and the
  re-review cap (`reviewer.max_code_reviews_per_pr`) paths unchanged.

## 3. Tests

- [ ] 3.1 With `max_revise_triggers_per_pr = None` (the default), N authorized
  ...
  message wording.
- [ ] 3.2 With `max_revise_triggers_per_pr = Some(n)`, the existing behavior holds:
...
- [ ] 3.3 Config: absent `max_revise_triggers_per_pr` resolves to `None`; an
  explicit integer in config resolves to `Some(n)`.

## 4. Validation

- [ ] 4.1 `cd autocoder && cargo test --bin autocoder` (the suite is known-flaky
  under parallel load — re-run / isolate any failure before treating it as real).
- [ ] 4.2 `openspec validate human-revise-cap-opt-in --strict` from the repo root.
