...
- [ ] 1.1 In the revision dispatcher's `Completed` / dirty-tree branch (the path
...
- [ ] 1.2 Reuse the existing marker-delete logic (the `ClearRevisionMarker`
...
- [ ] 1.3 Do NOT clear the marker on the other revision outcomes: the clean-tree
...
- [ ] 1.4 Leave the existing behavior intact: the agent is still instructed not to
...
- [ ] 2.1 A dirty-tree `Completed` revision with a `.needs-spec-revision.json`
...
- [ ] 2.2 A clean-tree declination (`Completed`, no change), a `Failed`, and a
  precondition-unmet revision each LEAVE a present `.needs-spec-revision.json`
  marker in place.
- [ ] 2.3 A dirty-tree `Completed` revision with NO marker present is a no-op (no
  error) — the clear is conditional on the marker existing.

## 3. Validation

- [ ] 3.1 `cd autocoder && cargo test --bin autocoder` (the suite is known-flaky
  under parallel load — re-run / isolate any failure before treating it as real).
- [ ] 3.2 `openspec validate revision-clears-needs-spec-revision-marker --strict`
  from the repo root.
