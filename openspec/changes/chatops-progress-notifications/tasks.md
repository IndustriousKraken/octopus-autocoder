...
- [ ] 6.4 Clear-on-success: at the top of `run_pass_through_commits`, right after `workspace::ensure_initialized` succeeds (NOT before), call `AlertState::clear(workspace)`. This way a transient init failure that recovers re-clears the state for the recovered category.
