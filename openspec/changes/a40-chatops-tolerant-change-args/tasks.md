...
- [ ] 2.1 In `autocoder/src/queue.rs`, add a public enum naming the marker filters the resolver supports:
  ```rust
  ...
  ```
- [ ] 2.2 Add `pub fn resolve_change_prefix(workspace: &Path, prefix: &str, scope: ChangePrefixMarkerScope) -> Result<String, ResolvePrefixError>` AND `pub enum ResolvePrefixError { NoMatch { scope: ChangePrefixMarkerScope }, MultiMatch { candidates: Vec<String> } }`. The resolver:
...
- [ ] 2.3 Add `impl ResolvePrefixError { pub fn to_operator_message(&self, prefix: &str) -> String }` so call sites format consistently:
...
- [ ] 2.4 Unit tests in `queue.rs`'s test module:
...
- [ ] 3.2 Apply the same integration to `handle_clear_perma_stuck` (scope `PermaStuck`).
- [ ] 3.3 Apply the same integration to `handle_ignore_for_queue` (scope `EitherBlocking` — the existing "has no operator-action marker" refusal in the handler is now performed AT resolution time; remove the post-resolution duplicate check since the resolver guarantees the marker exists).
- [ ] 3.4 Apply the same integration to `handle_clear_ignore_for_queue` (scope `IgnoreForQueue`).
- [ ] 3.5 Log line for each resolution success: `INFO control_socket: resolved partial change '<prefix>' → '<canonical>' for action <action>` (omitted when the prefix exactly equals the canonical slug — only log when the resolver did meaningful work).
...
- [ ] 4.1 Add an integration test that drives the chatops `handle_message` path with the literal text `@<bot> clear-revision <substr> \`a37\``, using a fixture workspace where `a37-foo/.needs-spec-revision.json` exists AND no other change carries the marker. Assert:
...
- [ ] 5.1 Update `docs/CHATOPS.md` near the recovery-verb table to call out the two new operator-friendly behaviors: surrounding backticks are tolerated, AND a leading-prefix slug is sufficient when only one change in the repo carries the action's relevant marker. One short paragraph; one example each. Avoid prescriptive tone — describe the relaxation, not "now you can finally...".
- [ ] 5.2 Update `docs/TROUBLESHOOTING.md` if the existing entries reference the invalid-change-slug error in a way that becomes stale (i.e., if there's an entry that says "the alert template wraps the slug in backticks; don't paste them" — that workaround is no longer needed). Otherwise skip.
- [ ] 5.3 No CONFIG.md update needed (no config knobs).
- [ ] 5.4 No CLI.md update needed (this is a chatops-layer change).

## 6. Acceptance gate

- [ ] 6.1 `cargo test` passes for the autocoder crate.
- [ ] 6.2 `openspec validate a40-chatops-tolerant-change-args --strict` passes.
- [ ] 6.3 Spot-check: the existing parser-layer scenarios (path-traversal, shell-metachar, oversize, valid-args) in `chatops-manager` still pass without modification — backtick stripping was inserted BEFORE the regex check, NOT instead of it.
