...
- [ ] Give the Slack event loop a frame deadline: when 90 seconds pass without
  ...
  unchanged.
- [ ] Make the disconnect log line name the frame deadline as the reason
  (distinct from ordinary Slack-initiated refreshes) so operators can spot a
  reaped half-open connection in the journal.
- [ ] Unit test: a connected stream that yields nothing for the deadline
  window produces the stream-error exit, and the reported reason names the
  frame deadline.
- [ ] Unit test: frames arriving with gaps shorter than the deadline —
  ...
  reaped.
- [ ] Unit test: cancellation still wins immediately while the loop is
