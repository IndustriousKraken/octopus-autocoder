...
`docs/CHATOPS.md`'s operator-recovery-commands section (where the `status` verb's reply shape is documented) SHALL include examples of every `currently:` line variant — including the open-PR park variant — AND explain the diagnostic value of each.
...
- **THEN** a paragraph explains that the `currently:` line distinguishes "audit in flight, just wait" from "parked on an open PR, merge or close it to resume" from "stale marker, need recovery to fire (or manual `rm`)" from "truly idle"
