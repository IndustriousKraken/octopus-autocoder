...
- [ ] 1.1 Add `scout`, `spec-it`, AND `clear-scout` to the recognized-verb list in the chatops inbound dispatcher.
- [ ] 1.2 Parse `@<bot> scout <repo-substring> [optional guidance]`:
...
- [ ] 1.3 Parse `@<bot> spec-it <item-number> [optional guidance]`:
...
- [ ] 1.4 Parse `@<bot> clear-scout <repo-substring>`:
...
- [ ] 1.5 Tests: each parse path, each refusal, AND the cross-thread scope check for spec-it.

## 2. Control-socket + state plumbing

- [ ] 2.1 In `autocoder/src/control_socket/actions.rs`, add `ScoutAction`, `SpecItAction`, AND `ClearScoutAction` variants.
- [ ] 2.2 New module `autocoder/src/state/scout_run.rs` defining `ScoutRunState { request_id, repo_url, guidance, head_sha_at_run, completed_at, thread_ts, channel, items: Vec<ScoutItem> }` AND `ScoutItem { id, category, title, body, source, tractability }`. Atomic-rename writes parallel to other state files.
- [ ] 2.3 Per-workspace path: `<workspace>/.state/scout_runs/<request_id>.json`. The "current" scout for a repo is the most-recent file by mtime.
- [ ] 2.4 Per-repo state extension: `pending_scout_requests: VecDeque<RequestId>` AND `pending_spec_it_requests: VecDeque<SpecItRequest>` queues.
- [ ] 2.5 Tests: state-file round-trip, queue enqueue/dequeue, atomic-write safety, "current" scout resolution by mtime.

## 3. Scout polling-loop handler

- [ ] 3.1 New module `autocoder/src/polling/scout.rs` exposing `process_pending_scout(repo_state, daemon_ctx) -> Result<()>`. Drains at most one request per iteration.
- [ ] 3.2 Gather inputs:
...
- [ ] 3.3 Invoke the executor in scout mode (WritePolicy::None; sandbox: Read, Glob, Grep, Bash including `gh`).
- [ ] 3.4 Parse the executor's response (JSON array of items). Validate:
...
- [ ] 3.5 Persist `ScoutRunState` AND post the rendered list to the request's thread. Rendering:
...
- [ ] 3.6 Append a closing note to the thread reply: `Reply with @<bot> spec-it <N> [optional guidance] to scope work on any item.`
- [ ] 3.7 If `gh api` failed, the closing note also names that issue-derived items were skipped this run.
- [ ] 3.8 Tests:
...
- [ ] 4.1 New module `autocoder/src/polling/spec_it.rs` draining `pending_spec_it_requests`.
...
- [ ] 5.1 In the polling-iteration's control-socket-action handling, process `ClearScoutAction`:
...
- [ ] 5.2 Tests: clear with multiple runs present; clear with no runs (reply uses count `0`); idempotent across re-invocations.

## 6. Scout prompt template

- [ ] 6.1 Create `prompts/scout.md`. Required content:
...
- [ ] 6.2 Register `PromptId::Scout` in `a24`'s loader registry.
- [ ] 6.3 `features.scout.prompt_path` per the `a24` uniform pattern; loader handles precedence.

## 7. Config integration

- [ ] 7.1 In `autocoder/src/config.rs`, extend `features` with `scout: { enabled: bool (default true), prompt_path: Option<String>, max_items: usize (default 30), include_issues: bool (default true), staleness_warn_days: u64 (default 7) }`.
- [ ] 7.2 Validate `max_items` is in `1..=50`; reject config with values outside that range.
- [ ] 7.3 Tests: defaults apply when block omitted; each field round-trips; invalid `max_items` fails fast.

## 8. Help-verb output

- [ ] 8.1 Update the help-verb's output to include `scout`, `spec-it`, AND `clear-scout` with their one-line descriptions. Order: scout under "chat-driven workflow"; spec-it noted as "scout-thread-only"; clear-scout under operator-recovery verbs.

## 9. Docs

- [ ] 9.1 `docs/CHATOPS.md`: add a `### scout` section under chat-driven workflow with syntax, output shape (compact description), AND the lifecycle-thread behavior. Add `### spec-it` noting it's scout-thread-only AND describes its translation to propose. Add `### clear-scout` under operator-recovery verbs.
- [ ] 9.2 `docs/OPERATIONS.md`: in the existing "Onboarding existing projects" OR a new "Finding things to work on" section, describe the scout → pick → spec-it flow as the recommended discovery loop.
- [ ] 9.3 `docs/CONFIG.md`: document the `features.scout.{enabled, prompt_path, max_items, include_issues, staleness_warn_days}` block.
- [ ] 9.4 Update the `a24` Prompt overrides table to include `PromptId::Scout` with `prompts/scout.md` AND `features.scout.prompt_path`.
- [ ] 9.5 `config.example.yaml`: include the `features.scout` block commented out.

## 10. Spec deltas

- [ ] 10.1 `openspec/changes/a25-scout-and-spec-it/specs/chatops-manager/spec.md` ADDs the three verb-parsing requirements.
- [ ] 10.2 `openspec/changes/a25-scout-and-spec-it/specs/orchestrator-cli/spec.md` ADDs the scout handler, spec-it handler, AND config-schema requirements.
- [ ] 10.3 `openspec/changes/a25-scout-and-spec-it/specs/project-documentation/spec.md` ADDs the docs requirement.

## 11. Verification

- [ ] 11.1 `cargo test` passes.
- [ ] 11.2 `openspec validate a25-scout-and-spec-it --strict` passes.
