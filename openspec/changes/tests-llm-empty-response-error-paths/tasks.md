...
- [ ] 1.1 `anthropic_errors_when_response_contains_no_text_block` — start a
...
- [ ] 1.2 `anthropic_errors_when_response_body_is_unparseable_json` —
...
- [ ] 2.1 `openai_compatible_errors_when_choices_array_is_empty` —
  ...
  `no choices`.
- [ ] 2.2 `openai_compatible_errors_when_response_body_is_unparseable_json`
  — register `POST /chat/completions` returning `200` with body
