...
- [ ] 1.1 `run_returns_err_on_subprocess_timeout` (in
...
- [ ] 1.2 Same test — after `run` returns, read the log file at
...
- [ ] 2.1 `run_returns_err_on_subprocess_timeout` (in
  ...
  `timeout`.
- [ ] 2.2 Same test — read the audit log file and assert it contains
...
- [ ] 3.1 `run_returns_err_on_subprocess_timeout` (in
...
- [ ] 3.2 Same test — verify the workspace state is unchanged: no
