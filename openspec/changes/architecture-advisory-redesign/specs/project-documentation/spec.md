...
Past the attention thresholds (file `800`, function `200` — both operator-configurable), a file or function is treated as a **structural defect to be addressed**, with the concern escalating as it climbs further over: the `architecture_advisor` audit surfaces it as a refactor recommendation grounded in the file's cohesion, not its raw line count. Duplicated logic — near-identical function bodies, OR one intent reimplemented across files — is likewise a structural defect; because it spans the whole tree rather than one file, surfacing it is a corpus-level concern, not the per-file advisor's.

These defects are surfaced **advisorily** — recommended by the `architecture_advisor` audit (which reasons about cohesion, not raw lines) AND noted by code review when a pass enlarges an over-budget file or function. A size finding SHALL NOT, on its own, block a pull request or a change from archiving; it is a maintainability signal that informs prioritization, not a correctness gate.
...
#### Scenario: A file far past the threshold is a structural defect to be addressed
- **WHEN** a source file's length is well past the file-line threshold
- **THEN** the project treats it as a structural defect to be addressed
- **AND** it is surfaced advisorily — recommended by the `architecture_advisor` audit AND noted by code review when a pass enlarges it — without, on size alone, blocking a pull request or a change from archiving
...
This requirement gives the project's largest historical bloat hotspot a durable structural contract: the `architecture_advisor` AND drift audits verify it on every run, so the module cannot silently re-accrete into one file.
