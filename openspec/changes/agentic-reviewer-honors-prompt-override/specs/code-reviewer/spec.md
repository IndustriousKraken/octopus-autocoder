...
The operator override (`reviewer.code_review.prompt_path`, or the legacy `reviewer.prompt_template_path`) SHALL be validated for readability at startup — a configured path that does not exist or fails to read returns `Err(_)` naming the path. The file's ROLE is per-transport: the ONESHOT path uses it as the full template in place of the default; the AGENTIC path includes its content as an operator guidance preamble, re-read per review — those semantics, including the use-time fail-loud posture for a file that becomes unreadable or empty after startup, are governed by the "Agentic reviewer mode" requirement, which this requirement references rather than restates.
...
- **WHEN** `reviewer.code_review.prompt_path` (or the legacy `reviewer.prompt_template_path`) is set in config
- **THEN** startup validates the file is readable, returning `Err(_)` naming the path when it is not
- **AND** the oneshot path reads the file as the full template AND uses it instead of the default; the agentic path includes its content as the operator guidance preamble per the "Agentic reviewer mode" requirement
...
**Operator prompt-override preamble.** When the reviewer prompt override is configured (`reviewer.code_review.prompt_path`, or the legacy `reviewer.prompt_template_path` — the same nested-then-legacy resolution the oneshot path uses, with the same startup readability validation per the "Default prompt template enforces code-quality scope" requirement), the agentic session's rendered prompt SHALL include the override file's content as an OPERATOR GUIDANCE PREAMBLE at the top of the prompt, ahead of every code-built section — which are all retained unchanged. This is the agentic-mode meaning of the override: guidance and calibration (e.g. when a concern warrants `should_request_revision: true`, review emphasis, house rules), NOT a replacement template — output-format mechanics belong to the `submit_review` contract, which the operator text cannot alter. The file is re-read per review so calibration edits apply on the next review; a configured override that has become unreadable or empty at review time SHALL fail the review loudly — the session is not spawned, the reviewer-failure alert fires naming the file and the cause — NEVER a silent fallback to the embedded guidance: an operator who configured calibration must be told when it is not applying. With no override configured, the prompt is rendered exactly as before.
