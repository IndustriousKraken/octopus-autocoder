...
- [x] 1.1 Remove from `autocoder/src/paths.rs`:
...
- [x] 1.2 Retain in `autocoder/src/paths.rs`:
...
- [x] 1.3 Add a module-level `//!` doc-comment documenting the threading convention: `Arc<DaemonPaths>` is constructed once at daemon startup AND threaded explicitly via constructor fields OR function parameters. No process-global cell. Reference the canonical orchestrator-cli "Production paths SHALL be threaded" requirement.

## 2. Daemon entrypoint plumbing

- [x] 2.1 In `autocoder/src/main.rs` (OR the equivalent entrypoint module — currently `cli/run.rs::run_daemon`), construct ONE `Arc<DaemonPaths>` at startup via the env-driven resolution.
- [x] 2.2 Remove the existing `paths::install_global(...)` call site in `autocoder/src/cli/run.rs`.
- [x] 2.3 Pass the `Arc<DaemonPaths>` to the top-level orchestrator constructor (OR equivalent — the struct that owns the polling tasks, chatops listener, AND control socket).
- [x] 2.4 Unit-test (entrypoint-shaped): a synthetic daemon startup constructs the `Arc<DaemonPaths>` AND passes it to the top-level type. The test does NOT need to actually run the daemon — it just verifies the constructor signature accepts the value.
...
- [x] 3.1 `autocoder/src/revisions.rs` — function-parameter pattern. Update `process_one_pr` AND any helpers reading `paths::current()` to accept `paths: &DaemonPaths`. Update polling-loop callers.
- [x] 3.2 `autocoder/src/alert_state.rs` — function-parameter pattern. Update `AlertState::load_or_default`, `AlertState::save`, AND any helpers.
- [x] 3.3 `autocoder/src/workspace.rs` — function-parameter pattern. Update `resolve_path` AND the helper at line 217.
- [x] 3.4 `autocoder/src/busy_marker.rs` — constructor-field pattern. Introduce a `BusyMarker { paths: Arc<DaemonPaths>, ... }` struct (OR equivalent) AND migrate the four call sites to methods.
- [x] 3.5 `autocoder/src/failure_state.rs` — function-parameter pattern.
- [x] 3.6 `autocoder/src/control_socket.rs` — constructor-field pattern. The control-socket handler struct gains a `paths: Arc<DaemonPaths>` field.
- [x] 3.7 `autocoder/src/audits/mod.rs` — constructor-field pattern. The audit framework's top-level type gains the field.
- [x] 3.8 `autocoder/src/audits/scheduler.rs` — constructor-field pattern. The scheduler struct gains the field.
- [x] 3.9 `autocoder/src/audits/threads.rs` — function-parameter pattern.
- [x] 3.10 `autocoder/src/proposal_requests.rs` — function-parameter pattern.
- [x] 3.11 `autocoder/src/changelog_requests.rs` — function-parameter pattern.
- [x] 3.12 `autocoder/src/executor/claude_cli.rs` — constructor-field pattern. The `ClaudeCliExecutor` struct gains the field.
- [x] 3.13 Update every caller in `autocoder/src/` that previously didn't need to pass `DaemonPaths` AND now does. Cascade upward until the caller has access to the threaded value (typically because it's a method of a struct that holds one, OR because main.rs handed it down).
