...
- **WHEN** an executor session returns a failed outcome BUT left a committable result (non-empty working-tree diff)
...
- **WHEN** it returns `Some(true)` for a failed session that DID produce a committable result
- **THEN** the orchestrator retries (the strategy has overridden the committable-result guard)
...
- **THEN** no whole-session retry is performed on a no-committable-result failure
