...
- [ ] 1.1 In `prompts/change-contradiction-check.md`, add a completeness
...
- [ ] 1.2 Add a `suggested_fix` directive: for each contradiction, produce a
...
- [ ] 1.3 Reframe the `submit_contradictions` JSON example as illustrative of a
  set AND include the `suggested_fix` field in the example shape.
- [ ] 1.4 Leave the existing NOT-a-contradiction guidance intact.

## 2. `[canon]` gate prompt: exhaustive sweep + wider scope + concrete suggested fix

- [ ] 2.1 In `prompts/change-vs-canonical-check.md`, add the completeness
  directive (as 1.1, against canon).
- [ ] 2.2 Widen the canon-reading instruction: replace "same — or related —
...
- [ ] 2.3 Add the `suggested_fix` directive (as 1.2): concrete edit plan per
...
- [ ] 2.4 Reframe the `submit_canon_contradictions` JSON example as illustrative
  of a set AND include the `suggested_fix` field.
- [ ] 2.5 PRESERVE the MODIFIED false-positive guardrail (a MODIFIED delta is
  never a contradiction with its own same-titled canonical requirement).

## 3. Schema + finding structs: `suggested_fix` field

- [ ] 3.1 Add `suggested_fix: String` to the `[in]` finding struct
...
- [ ] 3.2 Add `suggested_fix` to the `submit_contradictions` and
...
- [ ] 4.1 In the marker `revision_suggestion` builders
...
- [ ] 4.2 In the chatops alert rendering
...
- [ ] 5.1 Plumbing/completeness: a submission of N (>2) findings → all N appear in
  the marker `revision_suggestion` AND alert, none dropped. Assert finding
  presence/count, not wording.
- [ ] 5.2 Suggested-fix rendering: a finding with a non-empty `suggested_fix` →
  ...
  prose.
- [ ] 5.3 Back-compat parse: a `submit_*` payload omitting `suggested_fix` parses
  with the field defaulting to empty.
- [ ] 5.4 Do NOT add a test that greps the prompt files for phrasing (prompt
...
- [ ] 6.2 `openspec validate contradiction-gate-findings-complete-and-actionable
