...
- [ ] 10.3 Manual: run `cargo run -- sync-specs --rebuild --workspace .` against this autocoder repo (which has ~30 requirements unsynced from before the openspec-archive switch). Expect: all canonical specs regenerated; archive directory names unchanged (date prefixes preserved); `git diff openspec/specs/` shows the merged requirements; exit 0.
