...
- [ ] 1.1 In `autocoder/src/config.rs`, add top-level `Config.canonical_rag: Option<CanonicalRagConfig>`. `None` means the feature is disabled; `Some` with `enabled: false` is also disabled (explicit opt-out preserves the config block for documentation purposes).
- [ ] 1.2 Define `CanonicalRagConfig`:
  ```rust
  ...
  ```
- [ ] 1.3 Clamps + defaults: `top_k` clamps at `[1, 100]` with WARN. `api_key` AND `api_key_env` mutually exclusive (same pattern as `reviewer:` — inline wins with WARN if both set).
- [ ] 1.4 Update `config.example.yaml` AND the project-documentation config-coverage test list.
- [ ] 1.5 Tests: default `None` parses; explicit block with all fields parses; missing required field (e.g. `provider`) produces a clear error.

## 2. Embedding provider adapters

- [ ] 2.1 New module `autocoder/src/rag/embedding.rs`. Define:
  ```rust
  ...
  ```
- [ ] 2.2 Ollama adapter at `autocoder/src/rag/embedding/ollama.rs`:
...
- [ ] 2.3 OpenAI-compatible adapter at `autocoder/src/rag/embedding/openai_compatible.rs`:
...
- [ ] 2.4 Add `rig-core` dependency. Check current version at implementation time per `check-current-versions-not-training`. Rig's embedding builders MAY supersede our handwritten adapters; if so, prefer Rig's surface AND wire our trait through it.
- [ ] 2.5 Tests: both adapters against mocked HTTP responses; batch handling; error propagation (4xx, 5xx, network).

## 3. Markdown-aware chunking

- [ ] 3.1 New module `autocoder/src/rag/chunking.rs`. Define:
  ```rust
  ...
  ```
- [ ] 3.2 `PerRequirement` strategy: split on `### Requirement:` headers. Each chunk = the requirement's SHALL paragraph + every `#### Scenario:` block under it. The chunk's `text` includes the requirement title (so embeddings capture title semantics) + body. `requirement_title` is set; `scenario_titles` enumerates scenario headings.
- [ ] 3.3 `PerScenario` strategy (future; spec but don't implement in this change): finer chunking; one chunk per scenario. Tests can validate the contract; full implementation can ship in a follow-up.
- [ ] 3.4 `PerCapability` strategy (future; spec but don't implement): coarsest; one chunk per spec file. Tests validate the contract.
- [ ] 3.5 Tests:
...
- [ ] 4.1 New module `autocoder/src/rag/mod.rs`. Define:
  ```rust
  ...
  ```
- [ ] 4.2 `rebuild_for_workspace`: glob `<workspace>/openspec/specs/<cap>/spec.md`; for each, chunk + embed; populate entries.
- [ ] 4.3 `rebuild_capabilities`: for each named capability, remove existing entries with that capability slug, re-chunk + re-embed, append.
- [ ] 4.4 `query`: embed the query string; compute cosine similarity against every entry; return top-k sorted descending. Cosine similarity is float math over Vec<f32>; no external dep needed.
- [ ] 4.5 Per-workspace store registry. The daemon maintains `HashMap<workspace_basename, Arc<CanonicalRagStore>>` so multiple managed repos each have their own corpus.
- [ ] 4.6 Tests:
...
- [ ] 5.1 In the polling loop's workspace-init step (after `recreate_branch` succeeds for the first iteration of a workspace), check `config.canonical_rag` AND if `Some(c)` with `c.enabled == true`:
...
- [ ] 5.2 In the polling loop's post-archive step (after an iteration's commits land):
...
- [ ] 7.1 Edit `prompts/implementer.md` to add the canonical-RAG paragraph from the proposal. Place it in the "tools you have access to" section if one exists, or near the top of the file.
- [ ] 7.2 Embedded prompt re-loads at runtime via `include_str!` (existing pattern); no spec-side schema change.
- [ ] 7.3 Operators with custom implementer prompt overrides receive no automatic update — `docs/CONFIG.md`'s `executor.implementer_prompt_path` documentation gains a note about the new tool's availability so override-users can update their templates.

## 8. Install-wizard graduated-path flow

- [ ] 8.1 Extend `autocoder/src/cli/install.rs` with the graduated-path RAG prompt sequence per the proposal.
- [ ] 8.2 Detection logic:
...
- [ ] 8.4 Non-interactive mode: respect new flags `--rag-provider <ollama|openai_compatible|none>`, `--rag-base-url <url>`, `--rag-model <model>`, `--rag-api-key-env <name>`. Validation matches the interactive probe behavior.
- [ ] 8.5 Tests: wizard branches exercised via `ScriptedIo` + mocked HTTP responses.

## 9. Bundled docker-compose

- [ ] 9.1 Create `install/ollama-docker-compose.yml` per the proposal.
- [ ] 9.2 The file pulls `nomic-embed-text` as the default-startup model (cheaper than qwen3-embedding:4b; works on CPU; reasonable quality for the docker-quick-start use case). Operators upgrading hardware can edit the entrypoint to pull a bigger model.
- [ ] 9.3 The compose file is in-tree but NOT embedded into the binary — `autocoder install` copies it to the operator's config dir at install time. Stays editable post-install.
...
- [ ] 10.1 In `docs/CONFIG.md`, add a `## \`canonical_rag:\` (optional)` section documenting every field AND its default. Include a note that `canonical_rag.api_key_env` AND `canonical_rag.api_key` are mutually exclusive (same pattern as `reviewer:`).
- [ ] 10.2 In `docs/OPERATIONS.md`, add a "Canonical-spec RAG" section describing:
...
- [ ] 10.3 In `docs/CHATOPS.md`, add a one-line note in the implementer-flow section that the agent may call `query_canonical_specs` as part of its work; results show in the per-change run log.
- [ ] 10.4 In `docs/DEPLOYMENT.md`, add a "Self-hosted Ollama for RAG" subsection covering:
...
- [ ] 10.5 In `docs/CONFIG.md`'s `executor.implementer_prompt_path` row, add a note that operators with override templates can mention `query_canonical_specs` in their prompt OR ignore the new tool entirely.

## 11. Spec deltas

- [ ] 11.1 `openspec/changes/a21-canonical-spec-rag-via-mcp/specs/orchestrator-cli/spec.md` ADDs: `Canonical-spec RAG configuration AND pipeline`; `RAG re-embed cadence (workspace init AND post-archive)`; `Install-wizard graduated RAG-configuration flow`.
- [ ] 11.2 `openspec/changes/a21-canonical-spec-rag-via-mcp/specs/executor/spec.md` ADDs `MCP server exposes \`query_canonical_specs\` tool to the implementer`.
- [ ] 11.3 `openspec/changes/a21-canonical-spec-rag-via-mcp/specs/project-documentation/spec.md` ADDs `CONFIG.md, OPERATIONS.md, CHATOPS.md, AND DEPLOYMENT.md document the RAG configuration AND operator workflow`.

## 12. Verification

- [ ] 12.1 `cargo test` passes (new + existing).
- [ ] 12.2 `openspec validate a21-canonical-spec-rag-via-mcp --strict` passes.
- [ ] 12.3 `cargo clippy --all-targets --all-features -- -D warnings` produces no new warnings.
