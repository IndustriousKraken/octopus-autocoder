...
// The self-contained "Agentic reviewer transport (a58)" block lives in the
// `agentic` submodule. Re-exported here so callers outside the reviewer keep
// reaching these items at the `crate::code_reviewer::*` paths they already use.
mod agentic;
// Reached by callers outside the reviewer (polling_loop, revisions,
// control_socket, cli::run, mcp_askuser_server) — always in use.
pub(crate) use agentic::{
    AgenticReviewOutcome, REVIEWER_ROLE, apply_startup_cli_fallback,
    register_reviewer_submission_schema, run_agentic_review,
};
// Reached only by the reviewer's own `#[cfg(test)]` module (via `use super::*`);
// the re-export alias is unused in non-test builds. `payload_to_review_result`
// additionally backs a doc link in `mcp_askuser_server`, so the group stays
// present rather than being `#[cfg(test)]`-gated.
#[cfg_attr(not(test), allow(unused_imports))]
pub(crate) use agentic::{
    AGENTIC_REVIEW_ALLOWED_TOOLS, ReviewSessionRunner, agentic_review_allowed_tools,
    payload_to_review_result, render_agentic_review_prompt, resolve_reviewer_strategy,
    resolve_startup_reviewer_kind, reviewer_binary_on_path, run_agentic_review_with_runner,
    startup_reviewer_kind_decision, synthesize_agentic_per_change,
};
