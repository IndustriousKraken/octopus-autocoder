...
mod tests {
    ...
    /// app-under-test-e2e: the guide must tell a speccing agent that
    /// executable verification exists, because an agent that does not know it
    /// can require a runnable test writes prose assertions instead — the exact
    /// gap the feature closes. Asserted by meaning, matching the style above.
    #[test]
    fn octopus_md_documents_end_to_end_verification() {
        assert!(
            OCTOPUS_MD.contains("EXIT CODE"),
            "guide must name the exit code as the authoritative signal"
        );
        assert!(
            OCTOPUS_MD.contains("NOT asserted in prose"),
        ...
        assert!(
            OCTOPUS_MD.contains("cannot enable it from in here"),
            "guide must state the reader cannot enable this from inside the repository"
        );
        assert!(
            OCTOPUS_MD.contains("nothing else in this guide changes"),
            "guide must state its absence leaves the other protocols unchanged"
        );
    }
