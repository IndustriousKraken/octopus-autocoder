...
mod tests {
    ...
    async fn sandbox_settings_file_cleaned_up_after_run() {
        ...
        // a56 (Capture, no-MCP audit path): the audit goes through the
        // shared primitive with MCP disabled, so NO `.mcp.json` is written
        // into the workspace for the run.
        assert!(
            !workspace.join(".mcp.json").exists(),
            "audit run must not write a .mcp.json into the workspace"
        );
        if let Some(parent) = log_path.parent() {
            let _ = std::fs::remove_dir_all(parent.parent().unwrap_or(parent));
        }
