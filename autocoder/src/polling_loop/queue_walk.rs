...
pub(crate) async fn process_one_pending_change(
    ...
    // Canon-directing tasks are no longer caught by a mechanical `tasks.md`
    // scan here; the `[in]` gate's agentic session now reads `tasks.md` and
    // judges whether a task directs a canon edit (semantic judgment token
    // matching could not do). Deployments with the gate disabled fall back to
    // the archive-time duplicate-requirement abort.
