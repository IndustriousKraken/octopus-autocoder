//! `autocoder install --reconfigure` — re-prompt one section of an existing
//! install and patch its `config.yaml` in place. Extracted verbatim from
//! `install.rs`: the `--reconfigure <audits|reviewer|chatops>` flag, its
//! three-section allowlist, and every printed line of guidance are unchanged.
//! The shared wizard plumbing (the `WizardIo`/`SystemActions` traits, the
//! option tables, the per-prompt helpers, `serialize_config`, and
//! `confirm_diff_and_apply`) stays in `install.rs` and is imported here.
use crate::cli::install::{
    ...
    chatops_backend_label, confirm_diff_and_apply, idx_to_chatops_arg, idx_to_reviewer_arg,
    reviewer_arg_to_idx, reviewer_env_var, serialize_config,
};
use crate::config::{
