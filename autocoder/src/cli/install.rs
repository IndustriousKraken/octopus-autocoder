...
// The `--reconfigure` subsystem lives in the sibling `reconfigure` module;
// the section enum is re-imported here because the `InstallArgs.reconfigure`
// flag references it.
pub(crate) use crate::cli::reconfigure::ReconfigureSection;
...
/// the unit isn't loaded (or has no `--config` flag). Consumed by the
/// `--reconfigure` path (see [`crate::cli::reconfigure`]) AND the `run`
/// subcommand's config probe.
...
pub(crate) const CHATOPS_OPTIONS: &[&str] =
    &["none", "slack", "discord", "teams", "mattermost", "matrix"];
...
mod tests {
    use super::*;
    // The `--reconfigure` source now lives in the sibling `reconfigure`
    // module; its unit tests stay here (a pre-existing sibling test module)
    // and reach the moved functions by path.
    use crate::cli::reconfigure::{
        apply_in_place_patch, confirm_diff_and_apply, reconfigure_audits, reconfigure_chatops,
        reconfigure_reviewer, resolve_existing_config_path,
    };
