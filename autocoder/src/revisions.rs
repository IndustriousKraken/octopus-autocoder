...
// =====================================================================
// Per-PR comment dispatch
// =====================================================================
//
// The per-PR revision/code-review comment loop — the orchestration formerly
// carried by the single ~900-line `process_one_pr` here — lives in the
// `process_pr` submodule below, decomposed into phase helpers as a
// behavior-preserving split (PR outcomes AND outcome semantics are
// identical). `process_one_pr` is re-exported at this module's path so the
// one call site in `process_revision_requests_at` resolves unchanged; the
// submodule reaches every helper that stays here (`advance_seen`, the
// `compose_*` / `execute_*` / `apply_*` helpers, the state I/O, AND the
// shared types) via its own `use super::*`.
mod process_pr;
pub(crate) use process_pr::process_one_pr;
