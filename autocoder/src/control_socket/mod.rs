...
// The accreted `queue_*` enqueue/clear/revision handlers, the shared
// `enqueue_request` scaffold, AND the table-driven action dispatcher live in
// the `handlers` submodule; `dispatch_request` below delegates to it.
mod handlers;
...
pub async fn dispatch_request(line: &str, state: &ControlState) -> Value {
    ...
    handlers::dispatch(action.as_str(), &parsed, state).await
}
