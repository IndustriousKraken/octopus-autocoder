//! Slack ChatOps backend (officially supported).
//!
//! Wraps `chat.postMessage`, `conversations.replies`, and `auth.test`. The
//! constructor calls `auth.test` to cache `bot_user_id`, used to filter the
...
impl SlackBackend {
    ...
    /// Test-only constructor that targets a non-default API base URL.
    #[doc(hidden)]
...
mod tests {
    ...
    async fn new_caches_bot_user_id_on_success() {
        ...
        let chatops = SlackBackend::new_at(server.url(), "xoxb-fixture".into())
            .await
            .expect("auth ok");
    ...
    async fn fixture_chatops(server: &mut mockito::Server) -> SlackBackend {
        let _auth_mock = server
            ...
            .create_async()
            .await;
        SlackBackend::new_at(server.url(), "xoxb-test".into())
            .await
            .unwrap()
    }
    ...
    async fn provider_metadata() {
        let mut server = mockito::Server::new_async().await;
        let chatops = fixture_chatops(&mut server).await;
        assert_eq!(chatops.provider_name(), "slack");
        assert!(!chatops.is_experimental());
    }
}
