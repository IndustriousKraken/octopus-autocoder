//! The `send it` cascade and its audit/survey/issue-candidate/revision
//! state-machine logic (a03 / a010 / a29), extracted from
//! `operator_commands` so the dispatcher file keeps a single responsibility.
//!
//! [`OperatorCommandDispatcher::dispatch_send_it_on_audit`] is the cascade
//! entry point; it resolves the operator's `thread_ts` against the four
//! contexts in the canon-mandated order — audit → survey → issue-candidate →
//! revision — and falls through to the canonical untracked-thread refusal when
//! none matches. The three `try_send_it_on_*` fallbacks stay private to this
//! module (only the dispatcher calls them); the dispatcher itself is
//! `pub(crate)` so the command handler in the parent module still reaches it.
//!
//! As a child module of `operator_commands`, this file accesses the parent's
//! `OperatorCommandDispatcher` private fields and the `SEND_IT_REFUSE_*`
//! constants directly via the `use super::*` glob below; no field or const
...
impl OperatorCommandDispatcher {
    ...
    pub(crate) async fn dispatch_send_it_on_audit(
        &self,
        thread_ts: &str,
        repositories: &[RepoIdentity],
        submitter: &dyn ActionSubmitter,
