...
// The `send it` cascade (audit → survey → issue-candidate → revision) lives in
// the `send_it` child module; its dispatcher hangs off `OperatorCommandDispatcher`
// as a `pub(crate)` method reachable from the command handler below.
mod send_it;
