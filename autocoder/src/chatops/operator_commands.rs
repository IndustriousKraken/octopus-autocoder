...
pub fn parse_command(message: &str, bot_mention: &str) -> Option<OperatorCommand> {
    ...
    match verb.to_ascii_lowercase().as_str() {
        ...
        "wipe-workspace-confirm" | "confirm" => {
            // Either the explicit form (`@bot wipe-workspace-confirm coterie`)
...
mod tests {
    ...
    fn parse_status_happy_path() {
        let cmd = parse_command(&format!("{BOT} status coterie"), BOT).unwrap();
        assert_eq!(
            cmd,
            OperatorCommand::Status {
                repo_substring: "coterie".into()
            }
        );
    }
    ...
    fn parse_clear_perma_stuck_happy_path() {
        let cmd =
            parse_command(&format!("{BOT} clear-perma-stuck coterie a06-foo"), BOT)
                .unwrap();
        assert_eq!(
            cmd,
            OperatorCommand::ClearPermaStuck {
    ...
    fn parse_clear_revision_happy_path() {
        let cmd =
            parse_command(&format!("{BOT} clear-revision coterie a07-bar"), BOT).unwrap();
        assert_eq!(
            cmd,
            OperatorCommand::ClearRevision {
                repo_substring: "coterie".into(),
    ...
    fn parse_rebuild_specs_happy_path() {
        let cmd = parse_command(&format!("{BOT} rebuild-specs coterie"), BOT).unwrap();
        assert_eq!(
            cmd,
    ...
    fn parse_rebuild_specs_immediate_not_recognized() {
        ...
        let cmd = parse_command(&format!("{BOT} rebuild-specs coterie --immediate"), BOT);
        assert!(
            cmd.is_none(),
    ...
    fn parse_wipe_workspace_happy_path() {
        let cmd = parse_command(&format!("{BOT} wipe-workspace coterie"), BOT).unwrap();
        assert_eq!(
            cmd,
    ...
    fn parse_explicit_wipe_workspace_confirm() {
        let cmd =
            parse_command(&format!("{BOT} wipe-workspace-confirm coterie"), BOT).unwrap();
        assert_eq!(
            cmd,
            OperatorCommand::WipeWorkspaceConfirm {
                repo_substring: Some("coterie".into())
            }
        );
    }
    ...
    fn parse_missing_arg_returns_none() {
        ...
        assert!(parse_command(&format!("{BOT} clear-perma-stuck coterie"), BOT).is_none());
    ...
    fn parse_too_many_args_returns_none() {
        ...
        assert!(parse_command(&format!("{BOT} status coterie extra"), BOT).is_none());
    }

    #[test]
    fn parse_message_without_mention_returns_none() {
        // Don't drown random chat in error replies.
        assert!(parse_command("status coterie", BOT).is_none());
        assert!(parse_command("hello world", BOT).is_none());
        assert!(parse_command("@somebody-else status coterie", BOT).is_none());
    }
    ...
    fn parse_unknown_verb_returns_none() {
        ...
        assert!(parse_command(&format!("{BOT} pause coterie"), BOT).is_none());
        assert!(parse_command(&format!("{BOT} resume coterie"), BOT).is_none());
    ...
    fn parse_verb_is_case_insensitive() {
        for verb_form in ["status", "Status", "STATUS", "StAtUs"] {
            let cmd = parse_command(&format!("{BOT} {verb_form} coterie"), BOT)
    ...
    fn parse_whitespace_tolerance() {
        ...
        let cmd =
            parse_command(&format!("   {BOT}   status    coterie   "), BOT).unwrap();
        assert_eq!(
            cmd,
            OperatorCommand::Status {
                repo_substring: "coterie".into()
    ...
    fn match_repo_unique() {
        let repos = vec![
            repo("git@github.com:IndustriousKraken/coterie.git"),
            repo("git@github.com:IndustriousKraken/sound-cabinet.git"),
        ];
        match match_repo("coterie", &repos) {
            RepoMatch::Unique(r) => assert!(r.url.contains("coterie")),
    ...
    fn match_repo_case_insensitive() {
        let repos = vec![repo("git@github.com:IndustriousKraken/coterie.git")];
        match match_repo("COTERIE", &repos) {
            RepoMatch::Unique(r) => assert!(r.url.contains("coterie")),
            other => panic!("expected Unique, got {other:?}"),
        }
    ...
    fn format_no_match_lists_configured_repos() {
        let repos = vec![
            repo("git@github.com:owner/coterie.git"),
            repo("git@github.com:owner/sound-cabinet.git"),
        ];
        let out = format_no_match("gibberish", &repos);
        assert!(out.starts_with("✗ "));
        assert!(out.contains("gibberish"));
        assert!(out.contains("coterie"));
        assert!(out.contains("sound-cabinet"));
    }
    ...
    async fn dispatch_status_returns_formatted_reply() {
        ...
        submitter.set_response(
            ...
            serde_json::json!({
                "ok": true,
                "status": {
                    "url": "git@github.com:IndustriousKraken/coterie.git",
                    "perma_stuck_changes": [],
        ...
        assert!(reply.contains("git@github.com:IndustriousKraken/coterie.git"));
    ...
    async fn dispatch_clear_perma_stuck_on_unique_repo_submits_action() {
        ...
        let reply = dispatcher
            .handle_message(
                &format!("{BOT} clear-perma-stuck coterie a06-foo"),
                "C1",
                BOT,
                &fixture_repos(),
                &submitter,
        ...
        assert!(reply.contains("coterie"));
        let calls = submitter.calls();
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0]["action"], "clear_perma_stuck_marker");
        assert_eq!(
            calls[0]["url"], "git@github.com:IndustriousKraken/coterie.git"
        );
    ...
    async fn dispatch_clear_perma_stuck_propagates_action_error() {
        ...
        let reply = dispatcher
            .handle_message(
                &format!("{BOT} clear-perma-stuck coterie a99-nope"),
                "C1",
                BOT,
                &fixture_repos(),
                &submitter,
    ...
    async fn dispatch_no_match_replies_with_configured_list() {
        ...
        assert!(reply.contains("coterie"));
        assert!(reply.contains("sound-cabinet"));
    ...
    async fn wipe_workspace_two_step_confirm_happy_path() {
        ...
        submitter.set_response(
            ...
            serde_json::json!({
                "ok": true,
                "path": "/tmp/workspaces/github_com_IndustriousKraken_coterie",
        ...
        let warn = dispatcher
            .handle_message(
                &format!("{BOT} wipe-workspace coterie"),
                "C1",
                BOT,
                &fixture_repos(),
                &submitter,
    ...
    async fn wipe_workspace_reissue_replaces_prior_pending() {
        ...
        dispatcher
            .handle_message(
                &format!("{BOT} wipe-workspace sound-cabinet"),
                "C1",
                BOT,
                &fixture_repos(),
                &submitter,
        ...
        assert_eq!(
            wipe_call["url"], "git@github.com:IndustriousKraken/sound-cabinet.git",
