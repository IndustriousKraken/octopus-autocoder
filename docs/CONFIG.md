...
| `global_rules_check` | no | `disabled` | Opt-in gate for the change-vs-global-rules pre-flight — the `[rules]` gate (global-rules-gate), the corpus-parameterized sibling of `[canon]`. `disabled` (default) skips it. `enabled` runs alongside the `[in]`/`[canon]` gates, BEFORE the executor; it reads the change's spec-deltas AND the **global rule corpus** (see `global_rules.corpus`) and catches a change that would violate a portable engineering rule. Non-empty findings write `.needs-spec-revision.json` (naming each violated rule by id) and halt the queue walk. Enabling without **both** `global_rules_check_llm` AND a resolvable `global_rules.corpus` is a fail-fast startup error. Fails CLOSED. See [GLOBAL-RULES.md](GLOBAL-RULES.md) and [Pre-flight checks](OPERATIONS.md#pre-flight-checks). |
| `global_rules_check_prompt_path` | no | _embedded_ | Override for the built-in `[rules]`-check prompt template (embedded `prompts/global-rules-check.md`). An empty override file is rejected at use time. |
| `global_rules_check_llm` | required when enabled | _absent_ | LLM block for the `[rules]` gate. Same surface AND agentic transport as `change_internal_contradiction_check_llm` (see its row); pick a model independently. |
| `global_rules.corpus` | required when `global_rules_check` enabled | _absent_ | The global rule corpus location: a **local directory path** OR a **git repo URL** the daemon clones at startup (reused on later restarts). The directory holds project-agnostic rule files — flat OR grouped into register subdirectories — each a one-sentence rule + optional rationale; the file path (without extension) is the rule's stable id. The same corpus is read server-side (the enforcement guarantee) AND locally via `verify` (the accelerator). See [GLOBAL-RULES.md](GLOBAL-RULES.md) for the rule protocol and corpus layout. |
...
| `verifier_gate_retries` | no | `2` | Bounded retry of an agentic verifier-gate session ([in]/[canon]/[rules]/[out]) that ends with NO submission — the flaky case where a weak local model reads the spec then never calls its `submit_*` MCP tool. The same model+change submits non-deterministically, so a bounded retry catches the "sometimes works" case before the gate fails closed. Counts ADDITIONAL attempts: `2` (default) is up to 3 total attempts; `0` disables retry (one attempt — the historical behavior). Only the no-submission case retries — a successful submission, a timeout, AND an unregistered-strategy / CLI-unavailable error are NOT retried. This is the [gatekeepers-fail-closed](OPERATIONS.md#pre-flight-checks) standard's bounded-retry tolerance: after the bound is exhausted the gate still fails closed ([in]/[canon]/[rules] hold the change; [out] renders an advisory `## Spec Verification: FAILED TO RUN`), never fail-open. **Billing & visibility:** every re-attempt is a fresh, fully billed agentic session, so a higher value multiplies worst-case gate spend on a habitually flaky model. When a daemon-run gate invocation consumes at least one re-attempt, autocoder posts exactly ONE chatops notification for that invocation (never one per attempt, and not throttled) naming the gate, the change slug, attempts used vs. allowed, the final disposition (clean/findings/held), and the model — so rising spend is attributable to the model and you can act (e.g. switch models). Daemon-absent runs (`autocoder verify`, standalone gate runs) emit the same fields as a single WARN log line instead. |
...
| Field | Type | Description |
|---|---|---|
| `defaults` | `map<audit-slug, Cadence>` | Global default cadence per audit type. Audit slugs must match a registered type (currently `architecture_advisor`, `drift_audit`, `missing_tests_audit`, `security_bug_audit`, `documentation_audit`, `canon_contradiction_audit`, `canon_consolidation_audit`) OR the deterministic `spec_sync_audit` slug; typos fail at config load with a list of known slugs. Operators can re-prompt these cadences via `autocoder install --reconfigure audits` as an alternative to editing YAML directly — see [docs/CLI.md](CLI.md) for the full flag reference. |
...
| Field | Type | Description |
|---|---|---|
| `prompt_path` | `path` (optional) | Override the audit's embedded LLM prompt template. Used by `drift_audit` (embedded default: `prompts/drift-audit.md`), `missing_tests_audit` (embedded default: `prompts/missing-tests-audit.md`), `security_bug_audit` (embedded default: `prompts/security-bug-audit.md`), `architecture_advisor` (embedded default: `prompts/architecture-advisor.md`), and `documentation_audit` (embedded default: `prompts/documentation-audit.md`). An empty file is rejected at audit invocation so the daemon does not feed an empty prompt to the wrapped CLI. |
...
| `extra` | `map<string, yaml>` | Free-form per-audit knobs. `architecture_advisor` reads `selector_threshold` (`u64`, default `500` — only examine files longer than N lines) and `candidate_cap` (`usize`, default `8` — examine at most the N longest candidates) from here. `missing_tests_audit` and `security_bug_audit` each read `max_proposals_per_run` (`u32`, default `2`) from here. `documentation_audit` reads two knobs: `readme_max_lines` (`usize`, default `200`) and `page_max_lines_without_toc` (`usize`, default `500`). `drift_audit` does not currently read any `extra` knobs; configure it via the top-level `prompt_path` and `notify_on_clean` fields under `audits.settings.<slug>`. |
...
| `AuditArchitectureAdvisor`       | `prompts/architecture-advisor.md`          | `audits.settings.architecture_advisor.prompt_path`           | —                                                          |
