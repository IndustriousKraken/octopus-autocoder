...
The `.needs-spec-revision.json` marker's `revision_suggestion` names one or more `tasks.md` tasks that direct an edit to the canonical specs. This detection lives in the `[in]` gate's agentic session (it reads the change's `tasks.md` alongside the spec deltas) — it is NOT a mechanical scan, so it only fires where the `[in]` gate is enabled.
...
**The check.** When the `[in]` gate runs (before the executor), its session judges — semantically, by reading the task — whether any task's EDIT TARGET is the canonical specs (`openspec/specs/`). A task that merely MENTIONS the canonical specs as context or justification, or references the change's OWN delta (`openspec/changes/<slug>/specs/…`), is NOT flagged. On a flag the executor is NEVER invoked. Where the `[in]` gate is disabled, the archive-time duplicate-requirement abort remains the mechanical backstop.

**Where to find the diagnosis.** Read `revision_suggestion` in the marker file — it names each offending task line and states the rule (the implementer implements code and tests only; the delta is folded at archive).
