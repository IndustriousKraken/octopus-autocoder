...
|---|---|---|---|---|
| `architecture_advisor` | Selects a bounded candidate set using a cheap, language-agnostic signal — whole-file line count — taking ONLY the longest files over a configurable pain threshold (`selector_threshold`, default `500`), up to a configurable cap (`candidate_cap`, default `8`); the line count is a SELECTOR ONLY (it decides which files the audit examines) and is NEVER emitted as a finding. There is no function-length, duplicate-signature, or duplicate-body metric, and no ignore-file mechanism. For each candidate it invokes the wrapped agent CLI read-only (`Read`, `Glob`, `Grep`, `Bash`) with the `architecture-advisor` prompt and the `submit_findings` MCP tool; the agent reads the file plus surrounding context to judge cohesion and placement, then returns a professional recommendation — whether to refactor, the nature of the problem (oversized, low-cohesion "junk drawer", a single oversized function, or a monolith better wrapped than split), and a concrete recommended action grounded in the project's own language/architecture/patterns. Each recommendation carries a `file` or `file:line-range` anchor; the prompt forbids snark and generic best-practice lecturing. Output is `AuditOutcome::Reported(findings)`, ranked and capped at `5`; a run that examines its candidates and finds none worth refactoring returns an evidenced "no recommendation" (empty `Reported`) and the audit-run log records the candidates examined. Triggers on HEAD change at the configured cadence. | Yes | `disabled` (opt-in via `audits.defaults` or per-repo) | `None` (read-only; sandbox blocks `Write`/`Edit`, post-hoc diff check reverts any sneaky writes) |
...
| `missing_tests_audit` | Invokes the wrapped agent CLI with a sandbox that allows `Write` and `Edit` under `openspec/changes/` only, plus the read tools. The agent surveys the source tree, identifies uncovered error paths / branches without assertions / obvious edge cases, and creates up to `max_proposals_per_run` (default `2`) new OpenSpec change directories under `openspec/changes/tests-*` proposing tests to fill those gaps. The audit validates each new change via `openspec validate --strict`, rejects invalid ones (deletes the directory), and commits the valid ones to the agent branch as `audit: missing-tests proposals (N unit(s))`. **Lane choice (a01):** the audit reads the canonical spec(s) for each gap's area and routes the unit by canon judgment — a gap implying a new/changed invariant goes to the spec lane (`openspec/changes/<slug>/`), while a behavior-preserving gap in already-correctly-specified code goes to the issue lane (`issues/<slug>/`, an `issue.md` + `tasks.md` with no `specs/`). The issue lane is offered ONLY when `features.issues` is enabled for the repository; with it disabled, the audit behaves exactly as before (spec lane only). Returns `AuditOutcome::SpecsWritten(names)` carrying the produced unit names across both lanes; per `a12-changes-have-precedence-over-audits`, the new changes wait for the NEXT iteration's `walk_queue` (audits run AFTER the pending queue walk), so the audit's creation commits ship in iteration N's PR and the implementer's commits ship in iteration N+1's PR. **Additive only:** the prompt forbids deleting or modifying existing tests (except factually broken ones). All produced changes use the `tests-` prefix so operators recognize audit-produced work at a glance. Triggers on HEAD change at the configured cadence. | Yes | `disabled` (opt-in via `audits.defaults` or per-repo) | `PlanningLanes` (sandbox allows `Write`/`Edit`; post-hoc diff check reverts anything outside BOTH planning lanes — `openspec/changes/` AND `issues/`) |
| `security_bug_audit` | Invokes the wrapped agent CLI with the same `PlanningLanes` sandbox as `missing_tests_audit`, but with a security-and-bug-detection prompt. The agent surveys the source tree for high-confidence security issues (injection, auth/authz mistakes, hard-coded secrets, unsafe deserialization, missing input validation, race conditions, resource leaks) and likely bugs (off-by-one, wrong operator, mishandled None/null, missing error propagation, panicking on attacker-controlled input). For each confirmed finding it creates a `fix-*` (bug fixes) or `secure-*` (security hardening) unit describing the fix the implementer should make. Up to `max_proposals_per_run` (default `2`) per invocation. **Lane choice (a01):** the audit reads the canonical spec(s) for each finding's area and routes the unit by canon judgment — a fix that changes an observable contract goes to the spec lane (`openspec/changes/<slug>/`, validated via `openspec validate --strict`), while a behavior-preserving fix to already-correctly-specified code goes to the issue lane (`issues/<slug>/`, an `issue.md` + `tasks.md` with no `specs/`, not openspec-validated). The issue lane is offered ONLY when `features.issues` is enabled for the repository; with it disabled, the audit behaves exactly as before (spec lane only). It commits the valid units as `audit: security-bug proposals (N unit(s))`. Returns `AuditOutcome::SpecsWritten(names)` carrying the produced unit names across both lanes; per `a12-changes-have-precedence-over-audits`, the new changes wait for the NEXT iteration's `walk_queue` (audits run AFTER the pending queue walk), so the audit's creation commits ship in iteration N's PR and the implementer + reviewer pipeline catches any LLM mistakes in iteration N+1's PR. The prompt aggressively filters low-confidence findings — a false positive becomes wasted implementer work. **Operator warning:** this audit can be noisy in early iterations on an unfamiliar codebase. Monitor the first few invocations and tighten the prompt (or disable the audit) if the false-positive rate is high. Triggers on HEAD change at the configured cadence. | Yes | `disabled` (opt-in via `audits.defaults` or per-repo) | `PlanningLanes` (sandbox allows `Write`/`Edit`; post-hoc diff check reverts anything outside BOTH planning lanes — `openspec/changes/` AND `issues/`) |
...
- **`OpenSpecOnly`** — sandbox allows `Write`/`Edit`; after `run()` returns every modified or new path must begin with `openspec/changes/`. A diff outside that prefix triggers the same failure handling. (`canon_consolidation_audit` keeps this policy — it evolves canon and is spec-lane by definition.)
- **`PlanningLanes`** (a01) — sandbox allows `Write`/`Edit`; after `run()` returns every modified or new path must begin with EITHER planning lane — `openspec/changes/` (the spec lane) OR `issues/` (the issues lane). A diff outside both prefixes triggers the same failure handling (revert + `audit_write_policy_violation` alert + state not advanced). The two bug/gap audits (`missing_tests_audit`, `security_bug_audit`) run under this policy so each finding can be routed to its lane by canon judgment; **issues are a first-class audit output**, not just a spec-change output. The issue lane is offered to the agent ONLY when `features.issues` is enabled for the repository (the structural allowlist still permits issue-lane writes regardless, so nothing the agent produces is left uncommitted).
...
audits:
  defaults:
    architecture_advisor: weekly         # disabled | daily | every-N-days | weekly | monthly | quarterly; selector → LLM judgment → ranked anchored refactor recommendations; HEAD-change gated; recommended weekly
  ...
  settings:
    architecture_advisor:
      prompt_path: null                  # path overriding the embedded prompts/architecture-advisor.md; null → embedded prompt
      notify_on_clean: false             # true → post a brief "no recommendation" chatops message when the audit finds nothing worth refactoring
      extra:
        selector_threshold: 500          # only examine files longer than N lines (default 500); the line count is a selector, never a finding
        candidate_cap: 8                 # examine at most the N longest candidates (default 8)
    drift_audit:
    ...
    missing_tests_audit:
      ...
      extra:
        max_proposals_per_run: 2         # cap on the number of new tests-* units created per invocation, across both planning lanes (default 2)
    security_bug_audit:
      ...
      extra:
        max_proposals_per_run: 2         # cap on the number of new fix-*|secure-* units created per invocation, across both planning lanes (default 2)
    documentation_audit:
...
repositories:
  - url: "git@github.com:my-org/repo.git"
    ...
    audits:
      architecture_advisor: every-3-days   # per-repo override of the global default
```
...
**Authoring-time gate-checking and self-heal (a02).** The two bug/gap spec-writing audits (`security_bug_audit`, `missing_tests_audit`) and the `canon_consolidation_audit` run the SAME `[in]`/`[canon]` verifier-gate checks the executor runs as pre-flights (see [Pre-flight checks](#pre-flight-checks) below) — but at AUTHORING time, against the change the audit just wrote, while the authoring agent's context is still live. The checks obey the SAME opt-in flags: an enabled gate runs at authoring time (here, self-healing) AND at implement time (the unchanged pre-executor backstop); a disabled gate runs at neither. After a written spec-lane change passes `openspec validate --strict`, the enabled `[in]` (change-internal) and `[canon]` (change-vs-canonical) checks run against it; a contradiction feeds the audit's existing validation-retry loop — the authoring agent is re-invoked with the finding (AND the permitted resolutions: align to canon, write a legible `MODIFIED` delta whose contract change is stated plainly in the proposal rationale, or convert the unit to an issue) appended to its prompt, bounded by `audits.max_validation_retries`. A clean rewrite commits; an unresolved contradiction at budget exhaustion fails the unit CLOSED — `AuditOutcome::DidNotComplete` (found-but-could-not-persist), surfaced via the audit-failure `🚫` notification, with NO commit. A clean sibling unit produced in the same run still commits. An audit-authored ISSUE (a unit that claims, by carrying no spec delta, to change no contract) is checked too: when the `[canon]` gate is enabled, the audit runs an authoring-time contract-change check (reads `issue.md` + the relevant canon) — if implementing the issue would require a canonical contract change, the unit is re-routed to the spec lane through the same retry loop; an unresolved case fails closed. This is the early complement to the always-on implement-time issue kick-back ("the issue-flavored implementer verifies against existing canon"). A gate that fails to RUN (transport/parse/no-submission) is fail-closed too: the unit is held, not committed, distinct from a clean result.

**`requires_head_change` semantics:** Audits that compute over the codebase (like `architecture_advisor`, `drift_audit`, `missing_tests_audit`, `security_bug_audit`, and `documentation_audit`) declare `requires_head_change = true`; the scheduler skips them when the base-branch HEAD SHA matches the recorded `last_run_sha`, regardless of cadence. Audits whose inputs are external (package registries, GitHub PR lists) return `false` and run on cadence alone.
...
- `AuditOutcome::SpecsWritten(names)` → one `🔍 <repo-url>: <audit_type> created proposal \`<change-slug>\` — <first line of ## Why>` chatops post per validated change (see [CHATOPS.md → Proposal-created audit notifications](CHATOPS.md#progress-notifications)). The notification fires AFTER `openspec validate --strict` passes for each proposal AND BEFORE the audit's `git commit` ships it, so operators see provenance for the `🚀 starting work on …` line that follows on the next polling iteration. The framework also logs an info line naming each created change. The notification is always sent (not gated by `notify_on_clean`); the advisory `Reported`-only audits (`architecture_advisor`, `drift_audit`, `documentation_audit`) never fire it.
...
3. **`@<bot> send it`** posted as a reply inside the audit's thread. The dispatcher validates the thread (tracked, fresh, status `open` or `triage-failed`), submits a `trigger_audit_action` to the daemon, and flips the state to `triage-pending`. The next polling iteration drains the triage queue: the executor runs in triage mode against the findings, explores the codebase, and routes the work by canon judgment into ONE of the two planning lanes. By default a finding becomes an **issue** — for an `architecture_advisor` refactor recommendation (a behavior-preserving cleanup) the triage drafts `issues/<slug>/` containing `issue.md` and `tasks.md` with NO `specs/` directory; it drafts a spec change (`openspec/changes/<slug>/`) ONLY when the cleanup requires altering an observable contract (public API, wire format, CLI surface) or surfaces a genuine new capability decision. The completion handler keeps EITHER the spec-lane subtree (`openspec/changes/<slug>/`) OR the issues-lane subtree (`issues/<slug>/`) — whichever the run produced — and reverts everything else by the existing per-path strategy. It opens exactly **one PR** (a spec PR or an issue PR) with a lane-appropriate commit subject (`audit-triage spec proposal from <audit_type>` or `audit-triage issue from <audit_type>`). Content in either subtree is success; no content in either subtree flips the audit-thread to `triage-failed`. On success the state flips to `acted` after the PR lands, and the implementer makes the edits on the next iteration once the operator merges the PR. See [CHATOPS.md → Acting on audit findings](CHATOPS.md#acting-on-audit-findings-send-it) for the full operator-facing surface.

The triage-spawned PR (spec or issue) is a normal autocoder-opened PR that participates in the existing PR-comment revision loop (see [Revising an open PR via comment](#revising-an-open-pr-via-comment) below). If the agent over-promoted a finding or under-specified the work, `@<bot> revise <text>` on the PR gets revisions through the standard channel — the same channel the spec-driven workflow uses for correcting any other autocoder-opened PR. Because the PR diff is confined to a single planning lane by construction, revisions stay scoped to the proposal.
...
- **Curated.** A maintainer commits `issues/<slug>/` to the base branch directly. The next iteration works it into a fix and opens a PR. Always available once the lane is on.
- **Public ingestion.** Each iteration the daemon reads the repository's open GitHub issues read-only, classifies and dedups each against open and archived issues, and posts a **candidate** to chatops — it queues nothing. A maintainer promotes a candidate by replying `send it` in its thread (the same promotion pattern as audit findings); only then does the daemon write `issues/<slug>/` and queue it. Public ingestion additionally requires `features.scout.include_issues` (on by default).
...
**What triggers it.** Four independent code paths can write this marker:
...
4. **Pre-flight canon-editing-tasks check.** A sibling of the a17 archivability check that inspects `tasks.md` CONTENT rather than delta headers. Before invoking the executor, autocoder scans the change's `tasks.md` for a task that directs a direct edit to the canonical specs — a mutation verb (apply, add, copy, write, edit, update, insert, append, paste, create, populate) paired with a canonical-specs target (the path segment `openspec/specs/`, OR the words `canon` / `canonical spec`). The implementer implements code and tests only; a change's spec delta lives in its own `specs/<capability>/spec.md` and is folded into canon by `openspec archive` automatically. A task that instead applies the delta to `openspec/specs/` makes the implementer pre-fold canon, after which `openspec archive` aborts on a duplicate requirement AND the change goes perma-stuck. On a flag, autocoder writes the marker with `canon_editing_tasks` populated (naming each offending task), posts the chatops alert, AND halts the queue — the executor is never invoked AND the `[in]`/`[canon]` gates do not run. A reference to the change's OWN delta (`openspec/changes/<slug>/specs/…`) or a read-only mention of canon is NOT flagged. **The fix:** remove the offending task from `tasks.md` (the delta itself is untouched) AND clear the marker. The `revision_suggestion` is auto-generated and names the offending task(s) + the rule.

All four code paths share the same `AlertCategory::SpecNeedsRevision` throttle (24-hour, same as perma-stuck) and the same operator-clears-the-marker recovery shape. The marker schema accommodates any (or several) populations: `unimplementable_tasks` for the agent-detected path, `unarchivable_deltas` for the a17 pre-flight path, `canon_editing_tasks` for the canon-editing-tasks pre-flight path, AND a free-form `revision_suggestion` carrying the contradictions narrative for the a19 path.
...
`unimplementable_tasks`, `unarchivable_deltas`, and `canon_editing_tasks` are all optional (each elided from the JSON when empty). Pre-spec markers with only `unimplementable_tasks` continue to deserialize unchanged.
...
2. **`.needs-spec-revision.json`.** The agent flagged unimplementable tasks, the a17 pre-flight rejected an unarchivable spec delta, OR the canon-editing-tasks pre-flight rejected a task directing a canon edit. Cleared via `@<bot> clear-revision <repo> <change>` (or by deleting the file directly).
...
| **3. Canon-editing-tasks check** | Mechanical `tasks.md`-CONTENT check (sibling of layer 2, which inspects delta headers): flags a task pairing a mutation verb (apply, add, copy, write, edit, update, insert, append, paste, create, populate) with a canonical-specs target (`openspec/specs/`, OR `canon` / `canonical spec`). The implementer does code+tests only; the delta is folded into canon by `openspec archive`, so a task pre-folding it makes the archive abort on a duplicate requirement. References to the change's OWN delta (`openspec/changes/<slug>/specs/…`) AND read-only canon mentions are NOT flagged. | Free (mechanical, sub-millisecond) | Always on | `.needs-spec-revision.json` written with `canon_editing_tasks` populated (naming each offending task). Operator action: remove the offending task from `tasks.md` (the delta is untouched) AND clear the marker. The gates do not run. |
| **4. Change-internal contradiction check — the `[in]` gate (a19/a59)** | Agentic semantic check: detects requirements within the same change that cannot all hold simultaneously (e.g. ADDED A "all secrets in env vars" + ADDED B "API key in `config.yaml`"). Runs as a wrapped CLI session that is given the change's spec-delta file **paths**, reads them on demand in a read-only sandbox, AND returns findings via the `submit_contradictions` MCP tool over the control socket — NOT a one-shot HTTP call. Cost is model-dependent. | Agentic CLI session | Opt-in: `executor.change_internal_contradiction_check: enabled` + `executor.change_internal_contradiction_check_llm:` block | Findings → `.needs-spec-revision.json` with `revision_suggestion` from the contradictions narrative. **Fail-CLOSED (held):** a CLI-unavailable / no-submission / malformed result logs a WARN AND **holds the change** — writes the marker with a structured `gate_error` (gate + cause) + posts a distinct "gate FAILED TO RUN — change held" alert. The change was NOT evaluated (NOT waved through); fix the gate AND `clear-revision` to retry. |
| **5. Change-vs-canonical contradiction check — the `[canon]` gate (a62)** | Agentic semantic check, same transport as the `[in]` gate, that catches a change contradicting an ALREADY-canonical requirement (a delta that re-specifies a behavior locked elsewhere in `openspec/specs/`). Reads the change's spec-deltas AND `openspec/specs/*/spec.md` (using `query_canonical_specs` when `canonical_rag` is enabled), returning via the `submit_canon_contradictions` MCP tool. | Agentic CLI session | Opt-in: `executor.change_canonical_contradiction_check: enabled` + `executor.change_canonical_contradiction_check_llm:` block | Same as the `[in]` gate: findings → `.needs-spec-revision.json` with `revision_suggestion`; a gate that cannot run → fail-CLOSED held marker (`gate_error`) + "FAILED TO RUN" alert. |
| **6. Change-vs-global-rules check — the `[rules]` gate (global-rules-gate)** | The corpus-parameterized **sibling of the `[canon]` gate**: SAME agentic transport, but it compares the change's spec-deltas against a corpus of project-AGNOSTIC **global rules** (portable engineering lessons — no tautological tests, no committed secrets, …) instead of this project's canon, AND a finding names the violated rule by its stable id. The rule corpus (configured via `executor.global_rules.corpus` — a local path or a git repo the daemon clones) is fed to the session; it returns via the `submit_rule_violations` MCP tool. See [docs/GLOBAL-RULES.md](GLOBAL-RULES.md). | Agentic CLI session | Opt-in: `executor.global_rules_check: enabled` + `executor.global_rules_check_llm:` block + a resolvable `executor.global_rules.corpus` (enabling without BOTH model AND corpus is a fail-fast startup error) | Same as the `[in]`/`[canon]` gates: findings → `.needs-spec-revision.json` with `revision_suggestion` naming each violated rule; a gate that cannot run → fail-CLOSED held marker (`gate_error`) + "FAILED TO RUN" alert. |
