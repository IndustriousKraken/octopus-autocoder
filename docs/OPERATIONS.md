...
**Durability.** A queued audit is consumed from `pending_audit_runs` only once it has actually run. A pass that skips (busy marker), returns early (workspace-init failure), or is bounded out (`max_audits_per_iteration: 0`) leaves the queued entry in place for a later iteration — an acknowledged request is never silently dropped. (The queue is in-memory; cross-restart persistence is a planned follow-on.)
