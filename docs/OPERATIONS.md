...
`unimplementable_tasks` AND `unarchivable_deltas` are optional (each elided from the JSON when empty). Pre-spec markers with only `unimplementable_tasks` continue to deserialize unchanged. A legacy `canon_editing_tasks` array is still accepted for back-compat (markers written before canon-directing-task detection moved into the `[in]` gate), but no current code path populates it — the gate renders such findings into `revision_suggestion` instead.
...
2. **`.needs-spec-revision.json`.** The agent flagged unimplementable tasks, the a17 pre-flight rejected an unarchivable spec delta, OR the `[in]` gate found a contradiction or a task directing a canon edit. Cleared via `@<bot> clear-revision <repo> <change>` (or by deleting the file directly).
