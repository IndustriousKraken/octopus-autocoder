...
3. **`@<bot> send it`** posted as a reply inside the audit's thread. The dispatcher validates the thread (tracked, fresh, status `open` or `triage-failed`), submits a `trigger_audit_action` to the daemon, and flips the state to `triage-pending`. The next polling iteration drains the triage queue: the executor runs in triage mode against the findings, explores the codebase, and routes the work by canon judgment into ONE of the two planning lanes. By default a finding becomes an **issue** — for an `architecture_advisor` refactor recommendation (a behavior-preserving cleanup) the triage drafts `openspec/issues/<slug>/` containing `issue.md` and `tasks.md` with NO `specs/` directory; it drafts a spec change (`openspec/changes/<slug>/`) ONLY when the cleanup requires altering an observable contract (public API, wire format, CLI surface) or surfaces a genuine new capability decision. The completion handler keeps EITHER the spec-lane subtree (`openspec/changes/<slug>/`) OR the issues-lane subtree (`openspec/issues/<slug>/`) — whichever the run produced — and reverts everything else by the existing per-path strategy. It opens exactly **one PR** (a spec PR or an issue PR) with a lane-appropriate commit subject (`audit-triage spec proposal from <audit_type>` or `audit-triage issue from <audit_type>`). Content in either subtree is success; no content in either subtree flips the audit-thread to `triage-failed`. On success the state flips to `acted` after the PR lands, and the implementer makes the edits on the next iteration once the operator merges the PR. See [CHATOPS.md → Acting on audit findings](CHATOPS.md#acting-on-audit-findings-send-it) for the full operator-facing surface.
